...
void N1CtrlFSM::determine_state( const ros::Time &now_time )
{
    ...
    if ( ( now_time - odom_data.rcv_stamp ).toSec() < param.msg_timeout.odom )
    {
        ...
        if ( odom_flag_str.size() > 0 )
        {
            ...
            {
                ROS_ERROR( "<N1CTRL> Invalid Odometry Flag!!!" );
                odom_state = ODOM_VALID;
            }
        }
        else
    ...
    {
        std::string tmpss = boost::str(
            boost::format(
                "[Odom timeout] now: %.3f recvStamp: %.3f msgStamp: %.3f" ) %
    ...
    bool cmd_is_timeout =
        ( now_time - cmd_data.rcv_stamp ).toSec() < param.msg_timeout.cmd;
    bool cmd_is_valid = cmd_flag_valid & cmd_id_renewed & cmd_is_timeout;

//    cout << "================" <<endl;
//    cout << "cmd_flag_valid = " << cmd_flag_valid <<endl  << "cmd_id_renewed = " << cmd_id_renewed <<endl << "cmd_is_timeout = " << cmd_is_timeout <<endl;

    if ( !cmd_is_valid )
    {
        dbgss << boost::format(
                     "[Command not valid] flag[%d] cmd_id[%d] last_id[%d]" ) %
                     cmd_data.trajectory_flag % cmd_data.trajectory_id %
                     last_command_id
              << endl;
    }
    dbgss << boost::format(
                 "[Command info] now: %.3f recvStamp: %.3f msgStamp: %.3f" ) %
                 now_time % cmd_data.rcv_stamp % cmd_data.msg.header.stamp
          << endl;

    bool exit_cmd_js_check[ 4 ] = {
        std::fabs( rc_data.roll ) > param.rc.exit_cmd_deadzone,
    ...
    dbgss << boost::format( "[Exit cmd js] [%d%d%d%d]" ) % exit_cmd_js_check[ 0 ] %
                 exit_cmd_js_check[ 1 ] % exit_cmd_js_check[ 2 ] %
                 exit_cmd_js_check[ 3 ]
          << endl;
    ...
    while ( 1 )
    {
        ...
        else if ( state == JS_CTRL )
        {
                 ...
                 js_ctrl_mode == JS_CTRL_MODE_RAW )
            {
                ...
                go_to_state( JS_CTRL );
                flush_last_command_id();
                ROS_INFO( "From JS_RAW to JS_FEEDBACK\nReset trajectory id to %d",
                          cmd_data.trajectory_id );
            }
...
void N1CtrlFSM::go_to_state( State_t ds ) // dest state
{
    state = ds;
    switch ( ds )
    {
    case DIRECT_CTRL:
        ROS_WARN( "[N1CTRL] Exit API mode!" );
        break;
    case JS_CTRL:
        controller.config_gain( param.hover_gain );
        ROS_INFO( "[N1CTRL] ---- Enter JS_CTRL mode" );
        set_hov_with_odom();
        break;
    case JS_NO_CTRL:
        ROS_WARN( "[N1CTRL] JS EMERGENCY!" );
        break;
    ...
    case CMD_HOVER:
        controller.config_gain( param.hover_gain );
        ROS_INFO( "[N1CTRL] ---- Enter CMD_HOVER mode." );
        set_hov_with_odom();
        break;
    case CMD_CTRL:
        controller.config_gain( param.track_gain );
        ROS_INFO( "[N1CTRL] ---- Enter CMD_CTRL mode." );
        break;
    case CMD_NO_CTRL:
        ROS_WARN( "[N1CTRL] CMD EMERGENCY!" );
        break;
    case CMD_RESET_POS_CTRL:
        controller.config_gain( param.hover_gain );
        ROS_WARN( "[N1CTRL] CMD LOOP-CLOSURE!" );
        set_hov_with_odom();
        break;
    default:
        ROS_ASSERT( false );
...
void N1CtrlFSM::leave_state( State_t cs ) // current state
{
    switch ( cs )
    {
    case CMD_CTRL:
        ...
        if ( cmd_data.trajectory_flag ==
             quadrotor_msgs::PositionCommand::TRAJECTORY_STATUS_COMPLETED )
        {
            ROS_INFO( "Trajectory #%u complete when leave CMD_CTRL",
                      cmd_data.trajectory_id );
            dbgss << "Trajectory #" << cmd_data.trajectory_id
                  << "complete when leave CMD_CTRL." << endl;
        }
        break;
    default:
        ROS_ASSERT( false );
...
void N1CtrlFSM::determine_axis_states()
{
    bool channel_out_of_deadzone[ 4 ];
    bool velo_in_stop_range[ 4 ];

    channel_out_of_deadzone[ 0 ] =
        !uav_utils::in_range( rc_data.pitch, param.rc.deadzone );
    channel_out_of_deadzone[ 1 ] =
        !uav_utils::in_range( rc_data.roll, param.rc.deadzone );
    channel_out_of_deadzone[ 0 ] =
        channel_out_of_deadzone[ 0 ] || channel_out_of_deadzone[ 1 ];
    channel_out_of_deadzone[ 1 ] =
        channel_out_of_deadzone[ 0 ] || channel_out_of_deadzone[ 1 ];

    channel_out_of_deadzone[ 2 ] =
        !uav_utils::in_range( rc_data.thr, param.rc.deadzone );
    channel_out_of_deadzone[ 3 ] =
        !uav_utils::in_range( rc_data.yaw, param.rc.deadzone );

    Eigen::Vector3d v_c =
        rotz( get_yaw_from_quaternion( odom_data.q ) ).transpose() * odom_data.v;
    ...
    dbgss << boost::format( " rc[%d%d%d%d] velo[%d%d%d%d]" ) %
                 channel_out_of_deadzone[ 0 ] % channel_out_of_deadzone[ 1 ] %
                 channel_out_of_deadzone[ 2 ] % channel_out_of_deadzone[ 3 ] %
                 velo_in_stop_range[ 0 ] % velo_in_stop_range[ 1 ] %
                 velo_in_stop_range[ 2 ] % velo_in_stop_range[ 3 ];
}

void N1CtrlFSM::go_to_axis_state( AXIS_State_t ds, int i )
{
    axis_states[ i ] = ds;
    switch ( ds )
    {
...
void N1CtrlFSM::determine_idling( const ros::Time &now_time )
{
    ...
    bool idling_rule_vodom =
        ( std::fabs( odom_data.v( 2 ) ) < param.idling.feedback_velo_limit );
    ...
    dbgss << boost::format(
                 "Idling cond: outer[%d] podom[%d] pdes[%d] vodom[%d] vdes[%d]" ) %
                 idling_rule_outer % idling_rule_podom % idling_rule_pdes %
                 idling_rule_vodom % idling_rule_vdes
          << endl;

    if ( idling_state == NOIDLING )
    {
        if ( idling_rule_podom && idling_rule_pdes && idling_rule_vodom &&
             idling_rule_vdes )
        {
            ROS_INFO( "[N1Ctrl] Go into idling automatically." );
            idling_state = AUTOIDLING;
            idling_start_time = now_time;
        }
        else if ( idling_rule_outer )
        {
            ROS_WARN( "[N1Ctrl] Go into idling by outer instruction. [%d%d%d%d]",
                      idling_rule_podom, idling_rule_pdes, idling_rule_vodom,
                      idling_rule_vdes );
    ...
    else if ( idling_state == AUTOIDLING )
    {
        ...
        {
            ROS_INFO( "[N1Ctrl] Leave idling automatically.[%d%d%d]",
                      idling_rule_podom, idling_rule_pdes, idling_rule_vdes );
    ...
    else if ( idling_state == OUTERIDLING )
    {
        ...
        {
            ROS_WARN( "[N1Ctrl] Leave idling by outer instruction." );
            idling_state = NOIDLING;
        }
    }
    else
