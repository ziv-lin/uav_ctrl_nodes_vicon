...
void N1CtrlFSM::process_control( const ros::Time &now_time )
{
    ...
    odom_data.omega = imu_data.omega;
    ...
    else
    {
     ...
     //cout << "Output  57 "<< Vector3d( u.yaw, u.pitch, u.roll ).transpose()*57.3 <<endl;
    ...
    //cout << "Output  65 "<< Vector3d( u.yaw, u.pitch, u.roll ).transpose()*57.3 <<endl;
    ...
    if(CONTROL_ANGLE)
        align_with_imu( u );
    //cout << "Output  66 "<< Vector3d( u.yaw, u.pitch, u.roll ).transpose()*57.3 <<endl;
    ...
    if ( idling_state == NOIDLING )
    {
        if ( ( state == JS_CTRL && axis_states[ 2 ] == FIX &&
               js_ctrl_mode == JS_CTRL_MODE_FEEDBACK ) // JS_CTRL hovering case
...
void N1CtrlFSM::process_no_control( Controller_Output_t &    u,
                                    SO3_Controller_Output_t &u_so3 )
{
    u.roll = 0.0;
    u.pitch = 0.0;
...
void N1CtrlFSM::process_raw_control( Controller_Output_t &    u,
                                     SO3_Controller_Output_t &u_so3 )
{
    double des_yaw =
        yaw_add( get_yaw_from_odom(), -( rc_data.yaw * param.rc.yaw_scale ) );
...
void N1CtrlFSM::process_idling_control( Controller_Output_t &    u,
                                        SO3_Controller_Output_t &u_so3,
                                        double                   idling_lasting_time )
{
    u.roll = 0.0;
    u.pitch = 0.0;
    u.yaw = get_yaw_from_odom();
    if ( idling_lasting_time < param.idling.landing_timeout )
    {
        u.thrust = param.mass * param.gra / param.full_thrust *
                   param.idling.landing_thrust_percent;
    }
    else
    {
...
void N1CtrlFSM::process_hover_control( Controller_Output_t &    u,
                                       SO3_Controller_Output_t &u_so3 )
{
    Desired_State_t des;
...
void N1CtrlFSM::process_break_control( Controller_Output_t &    u,
                                       SO3_Controller_Output_t &u_so3 )
{
    Desired_State_t des;
    des.p = odom_data.p;
...
void N1CtrlFSM::process_cmd_control( Controller_Output_t &    u,
                                     SO3_Controller_Output_t &u_so3 )
{
    Desired_State_t des;
    des.p = cmd_data.p;
...
void N1CtrlFSM::process_js_control( Controller_Output_t &    u,
                                    SO3_Controller_Output_t &u_so3 )
{
    Desired_State_t des;
    Vector3d        des_v;
