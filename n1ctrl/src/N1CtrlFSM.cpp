...
N1CtrlFSM::N1CtrlFSM( Parameter_t &param_, Controller &controller_,
...
void N1CtrlFSM::process()
{
    ...
    {
        ROS_ERROR( "RC lost for %3f seconds!!!! Exit...",
                   ( now_time - rc_data.rcv_stamp ).toSec() );
        dbgss << "RC lost for " << ( now_time - rc_data.rcv_stamp ).toSec()
              << " seconds!!!! EXIT......";
    ...
    if ( rc_data.enter_command_mode )
    {
        ...
        ROS_INFO( "[N1CTRL] Traj trigger signal sent." );
        if ( state == DIRECT_CTRL )
        {
            rc_data.check_enter_command_mode();
            ROS_WARN( "[N1ctrl] IN DIRECT, Sending TRIGGER!" );
        }
    }
    ...
    if ( state == JS_NO_CTRL || state == JS_RESET_POS_CTRL ||
         state == CMD_NO_CTRL || state == CMD_RESET_POS_CTRL )
    {
...
bool N1CtrlFSM::rc_is_received( const ros::Time &now_time )
{
    return ( now_time - rc_data.rcv_stamp ).toSec() < param.msg_timeout.rc;
}

// ---- tools ----
double N1CtrlFSM::get_yaw_from_odom()
{
    return get_yaw_from_quaternion( odom_data.q );
}

void N1CtrlFSM::get_des_from_js( Vector3d &des_v, double &dyaw )
{
    Vector3d v_c( rc_data.pitch * param.rc.hori_velo_scale,
...
void N1CtrlFSM::align_with_imu( Controller_Output_t &u )
{
...
void N1CtrlFSM::set_hov_with_odom()
{
...
void N1CtrlFSM::flush_last_command_id()
{
    last_command_id = cmd_data.trajectory_id;
}

std::string N1CtrlFSM::state_to_str()
{
    switch ( state )
    {
    case DIRECT_CTRL:
    ...
    case JS_CTRL:
        return std::string( "JS-CTRL" );
    case JS_NO_CTRL:
        return std::string( "JS-NO" );
    case JS_RESET_POS_CTRL:
        return std::string( "JS-LC" );
    case CMD_HOVER:
        return std::string( "CMD-HOV" );
    case CMD_CTRL:
        return std::string( "CMD-CTRL" );
    case CMD_NO_CTRL:
        return std::string( "CMD-NO" );
    case CMD_RESET_POS_CTRL:
        return std::string( "CMD-LC" );
    default:
        ROS_ASSERT( false );
    }
    return std::string();
...
std::string N1CtrlFSM::axis_state_to_str()
{
    std::string s;
    for ( int i = 0; i < 4; ++i )
    {
...
void N1CtrlFSM::set_work_mode( Work_Mode_t mode )
{
    work_mode = mode;
    if ( work_mode == SIMULATION )
    {
        ROS_INFO( "[N1CTRL] work mode set to SIMULATION" );
    }
    else if ( work_mode == SIM_WITHOUT_RC )
    {
        ROS_INFO( "[N1CTRL] work mode set to SIM_WITHOUT_RC" );
    }
    else if ( work_mode == REALTIME )
    {
        ROS_INFO( "[N1CTRL] work mode set to REALTIME" );
    }
    else
    {
...
void N1CtrlFSM::publish_desire( const Desired_State_t &des )
{
    geometry_msgs::PoseStamped msg;
...
void N1CtrlFSM::set_js_ctrl_mode( JS_CTRL_Mode_t mode )
{
    js_ctrl_mode = mode;
    if ( js_ctrl_mode == JS_CTRL_MODE_RAW )
    {
        ROS_INFO( "[N1CTRL] js_ctrl_mode set to JS_CTRL_MODE_RAW" );
    }
    else if ( js_ctrl_mode == JS_CTRL_MODE_FEEDBACK )
    {
        ROS_INFO( "[N1CTRL] js_ctrl_mode set to JS_CTRL_MODE_FEEDBACK" );
    }
    else
    {
        ROS_ASSERT( false && "INVALID LOGIC BRANCH." );
