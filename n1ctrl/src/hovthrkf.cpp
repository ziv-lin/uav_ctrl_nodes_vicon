...
void HovThrKF::init()
{
    ...
    ROS_ASSERT_MSG( mass > 0.1 && max_force > 9.8, "mass=%f max_force=%f", mass,
                    max_force );
...
void HovThrKF::update( double a )
{
    ...
    {
        ROS_WARN( "[n1ctrl.hover] Reach percent_lower_limit %f", x( 0 ) );
    }
    if ( x( 0 ) == param.hover.percent_higher_limit )
    {
        ROS_WARN( "[n1ctrl.hover] Reach percent_higher_limit %f", x( 0 ) );
    }
}

double HovThrKF::get_hov_thr() { return x( 0 ); }
