...
TEST( GeometryUtilsDouble, Rotation )
{
    ...
    ASSERT_TRUE(
        v.isApprox( R_to_ypr( rotz( v( 0 ) ) * roty( v( 1 ) ) * rotx( v( 2 ) ) ), DOUBLE_EPS ) );

    double yaw = 30.0;
    ASSERT_DOUBLE_EQ(
        30.0, toDeg( get_yaw_from_quaternion( yaw_to_quaternion( toRad( yaw ) ) ) ) );
}
...
TEST( GeometryUtilsFloat, Rotation )
{
    ...
    ASSERT_TRUE(
        v.isApprox( R_to_ypr( rotz( v( 0 ) ) * roty( v( 1 ) ) * rotx( v( 2 ) ) ), FLOAT_EPS ) );
