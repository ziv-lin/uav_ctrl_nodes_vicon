...
double Polynomial::getValue( const double in ) { return Evaluate( in ); }

eigen_utils::Vector Polynomial::getRealRoot( double _y )
{
...
eigen_utils::Vector Polynomial::getRealRoot( double _y, double x_min,
                                             double x_max )
{
    eigen_utils::Vector realRoots, imagRoots;
...
double Polynomial::getOneRealRoot( double _y, double x_min, double x_max )
{
    eigen_utils::Vector realRoots, imagRoots;
...
eigen_utils::Vector Polynomial::getValue( const eigen_utils::Vector &in )
{
...
void Polynomial::setPolyOrder( int _order )
{
    m_order = _order;
    m_coeff.resize( m_order );
}

int Polynomial::getPolyOrder() const { return m_order; }

void Polynomial::setPolyCoeff( const eigen_utils::Vector &value )
{
    m_coeff = value;
}

void Polynomial::setPolyCoeff( int order_index, double value )
{
    m_coeff( order_index ) = value;
}

eigen_utils::Vector Polynomial::getPolyCoeff() const { return m_coeff; }

double Polynomial::getPolyCoeff( int order_index ) const
{
...
double Polynomial::Evaluate( double _x )
{
    double value_out = 0;
...
bool Polynomial::FindRoots( const double               y,
...
void Polynomial::FindLinearPolynomialRoots(
    const eigen_utils::Vector &polynomial, eigen_utils::Vector &real,
...
void Polynomial::FindQuadraticPolynomialRoots(
    const eigen_utils::Vector &polynomial, eigen_utils::Vector &real,
    eigen_utils::Vector &imag )
{
...
void Polynomial::BuildCompanionMatrix(
    const eigen_utils::Vector &polynomial,
...
void Polynomial::BalanceCompanionMatrix(
    eigen_utils::Matrix *companion_matrix_ptr )
{
    eigen_utils::Matrix &companion_matrix = *companion_matrix_ptr;
...
PolynomialFit::PolynomialFit( int _order ) : Polynomial( _order )
{
...
void PolynomialFit::loadSamples( const eigen_utils::Vector &x,
...
void PolynomialFit::loadSample( const Sample sample )
{
    samples.push_back( sample );
    data_size++;
}

void PolynomialFit::clearSamples()
{
    samples.clear();
    data_size = 0;
}

eigen_utils::Vector PolynomialFit::getCoeff()
{
    coeff = this->Fit();
...
Polynomial &PolynomialFit::getPolynomial() { return *poly; }

eigen_utils::Vector PolynomialFit::Fit()
{
