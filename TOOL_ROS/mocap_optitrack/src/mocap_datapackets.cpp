...
RigidBody::RigidBody() : NumberOfMarkers( 0 ), marker( 0 ) {}

RigidBody::~RigidBody() { delete[] marker; }
...
ModelDescription::ModelDescription() : numMarkers( 0 ), markerNames( 0 ) {}

ModelDescription::~ModelDescription() { delete[] markerNames; }

ModelFrame::ModelFrame()
    : markerSets( 0 ), otherMarkers( 0 ), rigidBodies( 0 ), numMarkerSets( 0 ),
      numOtherMarkers( 0 ), numRigidBodies( 0 ), latency( 0.0 ) {}
...
void MoCapDataFormat::parse()
{
    ...
    for ( int m = 0; m < model.numRigidBodies; m++ )
    {
        ...
        {
            model.rigidBodies[ m ].marker =
                new Marker[ model.rigidBodies[ m ].NumberOfMarkers ];
