...
void DjiRos::manually_leave_api_mode( bool need_release )
{
    ...
    ROS_ERROR( ANSI_COLOR_CYAN
               "[djiros] Manually exit api mode." ANSI_COLOR_RESET );
}

void DjiRos::on_authority_ack( Vehicle *vehicle, RecvContainer recvFrame,
                               DJI::OSDK::UserData userData )
{
    ...
    if ( ack.data == ErrorCode::ControlACK::SetControl::OBTAIN_CONTROL_SUCCESS )
    {
        ...
        p->sdk_control_flag = true;
              ...
              ErrorCode::ControlACK::SetControl::RELEASE_CONTROL_SUCCESS )
    {
        ROS_ERROR(
            ANSI_COLOR_CYAN
    ...
    else if ( p->api_trigger.getLevel() == 0 )
    {
        ...
        p->sdk_control_flag = false;
              ...
              ErrorCode::ControlACK::SetControl::OBTAIN_CONTROL_IN_PROGRESS )
    {
        ACK::getErrorCodeMessage( ack, __func__ );
        p->obtain_control( true );
    }
    else if ( ack.data ==
              ErrorCode::ControlACK::SetControl::RELEASE_CONTROL_IN_PROGRESS )
    {
        ACK::getErrorCodeMessage( ack, __func__ );
