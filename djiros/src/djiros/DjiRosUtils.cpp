...
bool Aligner::acquire_stamp( ros::Time &msg_stamp, uint32_t tick )
{
    ...
    if ( align_with_fmu )
    {
        if ( align_state == State_t::unaligned )
        {
            ...
            ROS_ERROR( "[djiros] start imu align, base = %.3f, tick = %d",
                       base_time.toSec(), tick );
            alignArray.clear();
            return false;
        }
        ...
        if ( align_state == State_t::aligning )
        {
            ...
            return false;
            // Here we do not do following tasks because I find that the dt varies
            // largely,
            ...
            else
            {
                ...
                {
                    double dt =
                        ( it->time - ( base_time + _TICK2ROSTIME( it->tick ) ) ).toSec();
                    printf( "dt=%.3f\n", dt * 1000.0 );
        ...
        if ( align_state == State_t::aligned )
        {
            ...
            if ( std::fabs( dt ) > TIME_DIFF_ALERT )
            {
                static int cnt = 0;
                ++cnt;
                ROS_WARN_THROTTLE( 1.0, "[djiros] SysTime - TickTime = %.0f ms [%d]",
                                   dt * 1000, cnt );
            }
