 ...
 */
void DJISDKNode::flightControl( uint8_t flag, float xSP, float ySP, float zSP,
                                float yawSP )
{
    ...
    if ( FRAME == Control::HORIZONTAL_GROUND )
    {
        ...
        {
            ROS_WARN_THROTTLE( 1.0, "GROUND frame is specified, but thrust command is "
                                    "generated in body frame" );
            zCmd = zSP;
        }
    }
...
void DJISDKNode::flightControlPxPyPzYawCallback(
    const sensor_msgs::Joy::ConstPtr &pMsg )
{
    uint8_t flag = ( Control::VERTICAL_POSITION | Control::HORIZONTAL_POSITION |
                     Control::YAW_ANGLE | Control::HORIZONTAL_GROUND |
    ...
    const sensor_msgs::Joy::ConstPtr &pMsg )
{
    ...
    printf("%s , %d \r\n", __FILE__, __LINE__);
    uint8_t flag =
        ( Control::VERTICAL_POSITION | Control::HORIZONTAL_ANGULAR_RATE |
          Control::YAW_RATE | Control::HORIZONTAL_BODY | Control::STABLE_ENABLE );
