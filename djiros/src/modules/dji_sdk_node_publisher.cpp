...
void DJISDKNode::SDKBroadcastCallback( Vehicle *vehicle, RecvContainer recvFrame,
                                       DJI::OSDK::UserData userData )
{
    ( ( DJISDKNode * ) userData )->dataBroadcastCallback();
}

void DJISDKNode::dataBroadcastCallback()
{
...
void DJISDKNode::publish10HzData( Vehicle *vehicle, RecvContainer recvFrame,
                                  DJI::OSDK::UserData userData )
{
...
void DJISDKNode::publish50HzData( Vehicle *vehicle, RecvContainer recvFrame,
                                  DJI::OSDK::UserData userData )
{
    DJISDKNode *p = ( DJISDKNode * ) userData;
...
void DJISDKNode::publish100HzData( Vehicle *vehicle, RecvContainer recvFrame,
                                   DJI::OSDK::UserData userData )
{
    DJISDKNode *p = ( DJISDKNode * ) userData;
    ...
    if ( p->align_time_with_FC )
    {
        if ( p->curr_align_state == ALIGNED )
        {
            ...
            if ( dt > TIME_DIFF_ALERT )
            {
                ...
                cnt++;
                ROS_WARN_THROTTLE(
                    1.0, "[dji_sdk] ros::Time::now() - TickTime = %.0f ms [%d]",
                    dt * 1000, cnt );
            }
        }
        else
...
void DJISDKNode::publish400HzData( Vehicle *vehicle, RecvContainer recvFrame,
                                   DJI::OSDK::UserData userData )
{
    DJISDKNode *p = ( DJISDKNode * ) userData;
...
void DJISDKNode::alignRosTimeWithFlightController( ros::Time now_time,
                                                   uint32_t  tick )
{
