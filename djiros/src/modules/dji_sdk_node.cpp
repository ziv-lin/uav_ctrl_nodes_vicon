...
bool DJISDKNode::initVehicle( ros::NodeHandle &nh_private )
{
    bool threadSupport = true;
    // LinuxSerialDevice* linuxSerialDevice = new
    // LinuxSerialDevice(serial_device.c_str(),baud_rate);
...
bool DJISDKNode::initFlightControl( ros::NodeHandle &nh )
{
...
ACK::ErrorCode DJISDKNode::activate( int l_app_id, std::string l_enc_key )
{
    usleep( 1000000 );
...
bool DJISDKNode::initSubscriber( ros::NodeHandle &nh )
{
    gimbal_angle_cmd_subscriber =
        nh.subscribe< dji_sdk::Gimbal >( "dji_sdk/gimbal_angle_cmd", 10,
                                         &DJISDKNode::gimbalAngleCtrlCallback, this );
    gimbal_speed_cmd_subscriber = nh.subscribe< geometry_msgs::Vector3Stamped >(
        "dji_sdk/gimbal_speed_cmd", 10, &DJISDKNode::gimbalSpeedCtrlCallback,
        this );
    return true;
}

bool DJISDKNode::initPublisher( ros::NodeHandle &nh )
{
   ...
   */
    ...
    else if ( telemetry_from_fc == USE_SUBSCRIBE )
    {
        ...
        {
            ROS_INFO( "align_time_with_FC set to false. We will time stamp messages "
...
bool DJISDKNode::initDataSubscribeFromFC()
{
    ...
    Telemetry::TopicName topicList50Hz[] = {
        Telemetry::TOPIC_GPS_FUSED, Telemetry::TOPIC_STATUS_FLIGHT,
...
void DJISDKNode::cleanUpSubscribeFromFC()
{
...
bool DJISDKNode::validateSerialDevice( LinuxSerialDevice *serialDevice )
{
    ...
    if ( !serialDevice->setSerialPureTimedRead() )
    {
        ROS_ERROR( "Failed to set up port for timed read.\n"
                   ...
                   "even when the port is correctly set up. If you are absolutely "
                   "certain\n"
                   "your connections are okay, try commenting out L77-L89 in file "
                   "dji_sdk_node.cpp\n"
                   "and build again.\n" );
        return ( false );
    };
