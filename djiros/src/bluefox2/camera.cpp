...
Camera::Camera( ros::NodeHandle _param_nh )
    : pnode( _param_nh ), m_fast_mode( false ), m_hwsync_grab_count( 0 ),
      m_max_req_number( -1 ), m_verbose_output( false ),
      m_sync_offset_for_debug( 0.0 )
{
    ...
    ROS_ASSERT_MSG( cam_cnt <= MAX_CAM_CNT, "cam_cnt exceeds MAX_CAM_CNT(%d)",
                    MAX_CAM_CNT );
    ...
    for ( int i = 0; i < cam_cnt; i++ )
    {
        ...
        pnode.param( prefix + "topic", cs.topic,
                     boost::str( boost::format( "image%d" ) % i ) );
        ...
        pnode.param( prefix + "use_auto_exposure", cs.use_auto_exposure,
                     use_auto_exposure );
        pnode.param( prefix + "aec_desired_gray_value", cs.aec_desired_gray_value,
                     aec_desired_gray_value );
        pnode.param( prefix + "aec_control_delay_frame", cs.aec_control_delay_frame,
                     aec_control_delay_frame );
        pnode.param( prefix + "exposure_time_us", cs.exposure_time_us,
                     exposure_time_us );
    ...
    for ( auto &item : camSettings )
    {
        ...
        img_publisher[ cs.serial ] =
            pnode.advertise< sensor_msgs::Image >( cs.topic, 10 );
    ...
    m_offset_sub = pnode.subscribe< std_msgs::Float64 >(
        "sync_offset", 10,
...
bool Camera::is_slave_mode() const { return m_is_slave_mode; }

bool Camera::is_fast_mode() const { return m_fast_mode; }

bool Camera::isOK() { return ok; }

bool Camera::initSingleMVDevice( unsigned int id, const CameraSetting &cs )
{
    ROS_INFO( "Camera Found:  %s(%s)", devMgr[ id ]->family.read().c_str(),
    ...
    // Set Properties
    mvIMPACT::acquire::SettingsBlueFOX settings(
        devMgr[ id ] ); // Using the "Base" settings (default)
    ...
    // Auto exposure, modified controller for better results, be careful about the
    // minimum exposure
    // time
    if ( cs.use_auto_exposure )
    {
        ...
        ROS_INFO(
            "Auto Exposure w/ Max Exposure Time (us) :  %d . Desired Grey Value: "
            "%d",
            settings.cameraSetting.autoControlParameters.exposeUpperLimit_us.read(),
            settings.cameraSetting.autoControlParameters.desiredAverageGreyValue
                .read() );
    }
    else
    {
    ...
    if ( cs.has_hdr )
    {
        if ( cs.use_hdr )
        {
            ...
            ROS_INFO( "  Voltage (mV):      %d", settings.cameraSetting.getHDRControl()
                                                     .getHDRKneePoint( 0 )
                                                     .HDRControlVoltage_mV.read() );
            ROS_INFO( "  Parts per Million: %d", settings.cameraSetting.getHDRControl()
                                                     .getHDRKneePoint( 0 )
...
bool Camera::grab_image_data()
{
    ...
    for ( auto &item : ids )
    {
        ...
        ROS_INFO_COND( m_verbose_output, "imageRequestWaitFor %7.3f ms",
                       ( s2 - s1 ).toSec() * 1000.0 );
    }
    ...
    if ( status )
    {
        ...
        if ( ok_cnt == cam_cnt )
        {
            for ( auto &item : ids )
            {
                ...
                std::string Encoding = Channels == 1
                ...
                ROS_ASSERT( static_cast< int >( Step * Height ) ==
                ...
                img_buffer.at( serial ).header.frame_id =
                    boost::str( boost::format( "expose_us:%d" ) %
                                pRequest[ devIndex ]->infoExposeTime_us.read() );
            }
        ...
        // Check imu response
        {
            ...
            if ( m_hwsync->ack_queue.size() )
            {
                ...
                else
                {
                    ...
                    ROS_ERROR( "Cannot sync! Image capturing is too slow! Try to decrease "
                               "fps/exposure or "
                               "turn off aec!" );
                    // ros::shutdown();
                }
    ...
    while ( pnode.ok() )
    {
        ...
        if ( grab_image_data() )
        {
            ...
            ROS_ASSERT_MSG( sync_ack.seq == m_hwsync_grab_count,
                            "sync_ack.seq=%d, hwsync_grab_count=%d", sync_ack.seq,
                            m_hwsync_grab_count );
    ...
    while ( pnode.ok() )
    {
        ...
        if ( grab_result )
        {
            for ( auto &item : img_publisher )
            {
                ...
                // After artificial calibrated, we found that actually image is taken
                // one imu later
                // than triggered
                img_buffer.at( serial ).header.stamp =
                    capture_time + ros::Duration( m_sync_offset_for_debug );
                item.second.publish( img_buffer.at( serial ) );
            }
        }
        m_hwsync_grab_count++; // inc it whether grab success or failed.
