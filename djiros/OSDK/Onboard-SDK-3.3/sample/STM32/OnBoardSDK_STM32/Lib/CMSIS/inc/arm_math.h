 ...
 * square root.
 * As compared to most of the other functions in the CMSIS math library, the
 * fast math functions
 ...
 * <code>arm_mat_init_q31()</code>
 * and <code>arm_mat_init_q15()</code> for floating-point, Q31 and Q15 types,
 * respectively.
 *
 * \par
...
typedef enum {
  ...
  ARM_MATH_SINGULAR =
    -5, /**< Generated by matrix inversion if the input matrix is singular and
           cannot be inverted. */
 ...
 */
__STATIC_INLINE q31_t
clip_q63_to_q31(q63_t x)
{
 ...
 */
__STATIC_INLINE q7_t
clip_q31_to_q7(q31_t x)
{
 ...
 */
__STATIC_INLINE q31_t
__SMUSDX(q31_t x, q31_t y)
{

  return (
    (q31_t)(((short)x * (short)(y >> 16)) - ((short)(x >> 16) * (short)y)));
}
 ...
 */
__STATIC_INLINE q31_t
__SMLAD(q31_t x, q31_t y, q31_t sum)
{
 ...
 */
__STATIC_INLINE q31_t
__SMLADX(q31_t x, q31_t y, q31_t sum)
{
 ...
 */
__STATIC_INLINE q31_t
__SMLSDX(q31_t x, q31_t y, q31_t sum)
{
 ...
 */
__STATIC_INLINE q63_t
__SMLALD(q31_t x, q31_t y, q63_t sum)
{
 ...
 */
__STATIC_INLINE q63_t
__SMLALDX(q31_t x, q31_t y, q63_t sum)
{
 ...
 * @param[in] numTaps  Number of filter coefficients in the filter. Must be even
...
typedef struct
{
  uint32_t numStages; /**< number of 2nd order stages in the filter.  Overall
                         order is 2*numStages. */
  float32_t*
    pState; /**< Points to the array of state coefficients.  The array is of
               length 4*numStages. */
  float32_t*
    pCoeffs; /**< Points to the array of coefficients.  The array is of length
                5*numStages. */
 ...
 * @param[in,out] *S points to an instance of the Q15 PID structure.
 * @param[in] resetStateFlag  flag to reset the state. 0 = no change in state 1
 * = reset the state.
 * @return none.
 */
...
typedef struct
{
  ...
  uint16_t
    twidCoefModifier; /**< twiddle coefficient modifier that supports different
 ...
 * @param[in]      *S    points to an instance of the floating-point CFFT/CIFFT
 * structure.
 * @param[in, out] *pSrc points to the complex data buffer. Processing occurs
 * in-place.
 * @return none.
 ...
 * @param[in]      bitReverseFlag flag that enables (bitReverseFlag=1) or
 * disables (bitReverseFlag=0) bit reversal of output.
 * @return		The function returns ARM_MATH_SUCCESS if initialization
 * is
 ...
 * @return		arm_status function returns ARM_MATH_SUCCESS if
 * initialization is successful or ARM_MATH_ARGUMENT_ERROR if
 * <code>fftLenReal</code> is not a supported transform length.
 */
 ...
 * not a supported transform length.
 */
...
* ARM_MATH_ARGUMENT_ERROR if the requested subset is not in the range [0
* srcALen+srcBLen-2].
*/
 ...
 * @return        The function returns ARM_MATH_SUCCESS if initialization is
 * successful or ARM_MATH_LENGTH_ERROR if
 * the filter length <code>numTaps</code> is not a multiple of the interpolation
 * factor <code>L</code>.
 */
...
typedef struct
{
  ...
  q15_t* pState; /**< points to the state variable array. The array is of length
                    numStages+blockSize. */
  q15_t*
    pkCoeffs; /**< points to the reflection coefficient array. The array is of
                 length numStages. */
...
typedef struct
{
  ...
  float32_t*
    pvCoeffs; /**< points to the ladder coefficient array. The array is of
                 length numStages+1. */
 ...
 * @param[in] *pSrcB points to the second input sequence.
 * @param[in] srcBLen length of the second input sequence.
 * @param[out] *pDst points to the block of output data  Length 2 * max(srcALen,
 * srcBLen) - 1.
 * @return none.
 */
...
typedef struct
{
  uint16_t numTaps; /**< number of coefficients in the filter. */
  uint16_t
    stateIndex; /**< state buffer index.  Points to the oldest sample in the
                   state buffer. */
 ...
 * <code>S</code> points to an instance of the PID control data structure.
 * <code>in</code>
 ...
 * together in an instance data structure.
 * A separate instance structure must be defined for each PID Controller.
 * There are separate instance structure declarations for each of the 3
 ...
 * in each function must be considered.
...
__STATIC_INLINE q31_t
arm_pid_q31(arm_pid_instance_q31* S, q31_t in)
{
  q63_t acc;
  q31_t out;
...
__STATIC_INLINE q15_t
arm_pid_q15(arm_pid_instance_q15* S, q15_t in)
{
  q63_t acc;
  q15_t out;
 ...
 * two-coordinate time invariant vector.
 * Generally the Clarke transform uses three-phase currents <code>Ia, Ib and
 * Ic</code> to calculate currents
 * in the two-phase orthogonal stator axis <code>Ialpha</code> and
 * <code>Ibeta</code>.
 ...
 * (a,b).
 * and <code>Ia + Ib + Ic = 0</code>, in this condition <code>Ialpha</code> and
 * <code>Ibeta</code>
 * can be calculated using only <code>Ia</code> and <code>Ib</code>.
 *
 * The function operates on a single sample of data and each call to the
 * function returns the processed output.
 * The library provides separate functions for Q31 and floating-point data
 * types.
 * \par Algorithm
 * \image html clarkeFormula.gif
 * where <code>Ia</code> and <code>Ib</code> are the instantaneous stator phases
 * and
 * <code>pIalpha</code> and <code>pIbeta</code> are the two coordinates of time
 * invariant vector.
 * \par Fixed-Point Behavior
...
arm_clarke_f32(float32_t Ia, float32_t Ib, float32_t* pIalpha,
               float32_t* pIbeta)
{
 ...
 * phases and
 * <code>Ialpha</code> and <code>Ibeta</code> are the two coordinates of time
 * invariant vector.
 * \par Fixed-Point Behavior
 * Care must be taken when using the Q31 version of the Clarke transform.
 * In particular, the overflow and saturation behavior of the accumulator used
...
__STATIC_INLINE void
arm_inv_clarke_f32(float32_t Ialpha, float32_t Ibeta, float32_t* pIa,
                   float32_t* pIb)
{
 ...
 * torque components.
 * The Park transform can be used to realize the transformation of the
 * <code>Ialpha</code> and the <code>Ibeta</code> currents
 ...
 * components,
 * <code>pId</code> and <code>pIq</code> are rotor vector components and
 * <code>cosVal</code> and <code>sinVal</code> are the
...
__STATIC_INLINE void
arm_park_f32(float32_t Ialpha, float32_t Ibeta, float32_t* pId, float32_t* pIq,
 ...
 * The accumulator maintains 1.31 format by truncating lower 31 bits of the
 * intermediate multiplication in 2.62 format.
 * There is saturation on the addition and subtraction, hence there is no risk
 * of overflow.
 */

__STATIC_INLINE void
arm_park_q31(q31_t Ialpha, q31_t Ibeta, q31_t* pId, q31_t* pIq, q31_t sinVal,
             q31_t cosVal)
{
  q31_t product1,
    product2; /* Temporary variables used to store intermediate results */
 ...
 * components,
 * <code>Id</code> and <code>Iq</code> are rotor vector components and
 * <code>cosVal</code> and <code>sinVal</code> are the
 * cosine and sine values of theta (rotor flux position).
 * \par Fixed-Point Behavior
 * Care must be taken when using the Q31 version of the Park transform.
...
__STATIC_INLINE void
arm_inv_park_q31(q31_t Id, q31_t Iq, q31_t* pIalpha, q31_t* pIbeta,
 ...
 * input(x)
 * using linear interpolation of the input values x0, x1( nearest input values)
 * and the output values y0 and y1(nearest output values)
 *
 * \par Algorithm:
 * <pre>
 ...
 * value.
 * <code>S</code> points to an instance of the Linear Interpolate function data
 * structure.
 * <code>x</code> is the input sample value. The functions returns the output
 * value.
 *
 * \par
 * if x is outside of the table boundary, Linear interpolation returns first
 * value of the table
 * if x is below input range and returns last value of table if x is above
 * range.
 */
...
* table index and 20 bits for fractional part.
 ...
 * <code>in</code> is negative value and returns zero output for negative
 * values.
 */
 ...
 * The underlying function <code>f(x, y)</code> is sampled on a regular grid and
 * the interpolation process
 * determines values between the grid points.
 * Bilinear interpolation is equivalent to two step linear interpolation, first
 * in the x-dimension and then in the y-dimension.
 * Bilinear interpolation is often used in image processing to rescale images.
 * The CMSIS DSP library provides bilinear interpolation functions for Q7, Q15,
 * Q31, and floating-point data types.
 *
 * <b>Algorithm</b>
 * \par
 ...
 * <code>numRows*numCols</code> values.
 * The data table <code>pTable</code> is organized in row order and the supplied
 * data values fall on integer indexes.
...
__STATIC_INLINE float32_t
arm_bilinear_interp_f32(const arm_bilinear_interp_instance_f32* S, float32_t X,
                        float32_t Y)
{
  float32_t  out;
