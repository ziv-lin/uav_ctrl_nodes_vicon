/**
  ...
  *          This file contains the system clock configuration for STM32F4xx
  *devices,
  ...
  *Multiplier
  *                      and Divider factors, AHB/APBx prescalers and Flash
  *settings),
  *                      depending on the configuration made in the clock xls
  *tool.
  ...
  *SystemInit()
  *    function will do nothing and HSI still used as system clock source. User
  *can
  *    add some code to deal with this issue inside the SetSysClock() function.
  *
  * 4. The default value of HSE crystal is set to 25MHz, refer to "HSE_VALUE"
  *define
  *    in "stm32f4xx.h" file. When HSE is used as system clock source, directly
  *or
  *    through PLL, and you are using different crystal you have to adapt the
  *HSE
...
#ifdef VECT_TAB_SRAM
  SCB->VTOR =
    SRAM_BASE | VECT_TAB_OFFSET; /* Vector Table Relocation in Internal SRAM */
#else
  SCB->VTOR = FLASH_BASE |
 ...
 * HSE_VALUE(**)
  ...
  *
  *         (*) HSI_VALUE is a constant defined in stm32f4xx.h file (default
 * value
  *             16 MHz) but the real value may vary depending on the variations
  *             in voltage and temperature.
  *
  *         (**) HSE_VALUE is a constant defined in stm32f4xx.h file (default
 * value
 ...
 * STM324xG_EVAL/STM324x7I boards
  *         This SRAM will be used as program data memory (including heap and
 * stack).
  * @param  None
  * @retval None
  */
...
SystemInit_ExtMemCtl(void)
{
  ...
  /*
   ...
   FSMC_A11 |
   | PD4  <-> FSMC_NOE | PE2  <-> FSMC_A23  | PF2 <-> FSMC_A2  | PG2 <->
   FSMC_A12 |
   | PD5  <-> FSMC_NWE | PE3  <-> FSMC_A19  | PF3 <-> FSMC_A3  | PG3 <->
   FSMC_A13 |
   | PD8  <-> FSMC_D13 | PE4  <-> FSMC_A20  | PF4 <-> FSMC_A4  | PG4 <->
   FSMC_A14 |
   | PD9  <-> FSMC_D14 | PE5  <-> FSMC_A21  | PF5 <-> FSMC_A5  | PG5 <->
   FSMC_A15 |
   | PD10 <-> FSMC_D15 | PE6  <-> FSMC_A22  | PF12 <-> FSMC_A6 | PG9 <->
   FSMC_NE2 |
