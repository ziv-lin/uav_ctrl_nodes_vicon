...
#define ITM_TCR_GTSFREQ_Pos                                                    \
  10 /*!< ITM TCR: Global timestamp frequency Position */
...
#define MPU_RASR_ATTRS_Pos                                                     \
  16 /*!< MPU RASR: MPU Region Attribute field Position */
...
typedef struct
{
  ...
  __IO     uint32_t
           FPDSCR; /*!< Offset: 0x00C (R/W)  Floating-Point Default Status Control
                      Register        */
  __I uint32_t
...
#define FPU_FPCCR_LSPACT_Pos                                                   \
  0 /*!< FPCCR: Lazy state preservation active bit Position */
...
#define FPU_MVFR0_FP_rounding_modes_Pos                                        \
  28 /*!< MVFR0: FP rounding modes bits Position */
...
#define FPU_MVFR0_Short_vectors_Pos                                            \
  24 /*!< MVFR0: Short vectors bits Position */
...
#define FPU_MVFR0_FP_excep_trapping_Pos                                        \
  12 /*!< MVFR0: FP exception trapping bits Position */
...
#define FPU_MVFR0_Double_precision_Pos                                         \
  8 /*!< MVFR0: Double-precision bits Position */
...
#define FPU_MVFR0_Single_precision_Pos                                         \
  4 /*!< MVFR0: Single-precision bits Position */
...
#define FPU_MVFR0_A_SIMD_registers_Pos                                         \
  0 /*!< MVFR0: A_SIMD registers bits Position */
#define FPU_MVFR0_A_SIMD_registers_Msk                                         \
  (0xFUL                                                                       \
   << FPU_MVFR0_A_SIMD_registers_Pos) /*!< MVFR0: A_SIMD registers bits Mask   \
                                           */
...
NVIC_SetPriorityGrouping(uint32_t PriorityGroup)
{
  ...
  uint32_t PriorityGroupTmp =
    (PriorityGroup & (uint32_t)0x07); /* only values 0..7 are used          */
...
NVIC_EnableIRQ(IRQn_Type IRQn)
{
   ...
   * enable interrupt */
  NVIC->ISER[(uint32_t)((int32_t)IRQn) >> 5] = (uint32_t)(
    1 << ((uint32_t)((int32_t)IRQn) & (uint32_t)0x1F)); /* enable interrupt */
}
...
/** \brief  Encode Priority
   ...
   group,
    ...
    In case of a conflict between priority grouping and available
    priority bits (__NVIC_PRIO_BITS), the samllest possible priority group is
   set.
...
/** \brief  Decode Priority
    ...
    The function decodes an interrupt priority value with a given priority group
   to
    preemptive priority value and subpriority value.
    In case of a conflict between priority grouping and available
    priority bits (__NVIC_PRIO_BITS) the samllest possible priority group is
   ...
   the function \ref NVIC_GetPriority().
    \param [in]     PriorityGroup  Used priority group.
