 ...
 ===============================================================================
    [..]
      ...
      (+) A PVDO flag is available to indicate if VDD/VDDA is higher or lower
          ...
          line16 and can generate an interrupt if enabled through the EXTI
registers.
...
from
          ...
          retained even in Standby or VBAT mode when the low power backup
regulator
...
VDD)
          the backup SRAM is powered from VDD which replaces the VBAT power
supply to
          save battery life.
...
between
          ...
          the maximum frequency. This is done through
PWR_MainRegulatorModeConfig()
          function which configure VOS bit in PWR_CR register:
        (++) When this bit is set (Regulator voltage output Scale 1 mode
selected)
             the System frequency can go up to 168 MHz.
        (++) When this bit is reset (Regulator voltage output Scale 2 mode
selected)
 ...
 ===============================================================================
   ...
   *** Sleep mode ***
   ==================
    [..]
      (+) Entry:
        (++) The Sleep mode is entered by using the __WFI() or __WFE()
functions.
      (+) Exit:
...
contents
      are preserved.
      The voltage regulator can be configured either in normal or low-power
mode.
      To minimize the consumption In Stop mode, FLASH can be powered off before
      entering the Stop mode. It can be switched on again by software after
exiting
...
PWR_EnterSTOPMode(PWR_Regulator_LowPower,)
      ...
      (+) Exit:
        (++) Any EXTI Line (Internal or External) configured in Interrupt/Event
mode.
...
based
      ...
      The 1.2V domain is consequently powered off. The PLL, the HSI oscillator
and
      the HSE oscillator are also switched off. SRAM and register contents are
lost
      except for the RTC registers, RTC backup registers, backup SRAM and
Standby
      circuitry.
      ...
      (+) Entry:
        (++) The Standby mode is entered using the PWR_EnterSTANDBYMode()
function.
      (+) Exit:
        (++) WKUP pin rising edge, RTC alarm (Alarm A and Alarm B), RTC wakeup,
             tamper event, time-stamp event, external reset in NRST pin, IWDG
reset.
...
necessary to:
          (+++) Configure the EXTI Line 17 to be sensitive to rising edges
(Interrupt
...
event, it
             is necessary to:
          (+++) Configure the EXTI Line 21 to be sensitive to rising edges
(Interrupt
                or Event modes) using the EXTI_Init() function.
          (+++) Enable the RTC Tamper or time stamp Interrupt using the
RTC_ITConfig()
                function
          (+++) Configure the RTC to detect the tamper or time stamp event using
the
...
necessary to:
           (+++) Configure the EXTI Line 22 to be sensitive to rising edges
(Interrupt
                 or Event modes) using the EXTI_Init() function.
           (+++) Enable the RTC WakeUp Interrupt using the RTC_ITConfig()
function
...
RTC_WakeUpClockConfig(),
      ...
      (#) RTC auto-wakeup (AWU) from the Standby mode
        ...
        (++) To wake up from the Standby mode with an RTC alarm event, it is
necessary to:
          (+++) Enable the RTC Alarm Interrupt using the RTC_ITConfig() function
          (+++) Configure the RTC to generate the RTC alarm using the
RTC_SetAlarm()
                and RTC_AlarmCmd() functions.
        (++) To wake up from the Standby mode with an RTC Tamper or time stamp
event, it
             is necessary to:
          (+++) Enable the RTC Tamper or time stamp Interrupt using the
RTC_ITConfig()
...
RTC_TamperCmd()
                functions.
        (++) To wake up from the Standby mode with an RTC WakeUp event, it is
necessary to:
          (+++) Enable the RTC WakeUp Interrupt using the RTC_ITConfig()
function
          (+++) Configure the RTC to generate the RTC WakeUp event using the
 ...
 * consumption
  ...
  *            @arg PWR_Regulator_LowPower: STOP mode with regulator in low
 * power mode
 ...
 * event
  *                  was received from the WKUP pin or from the RTC alarm (Alarm
 * A
  *                  or Alarm B), RTC Tamper event, RTC TimeStamp event or RTC
 * Wakeup.
  *                  An additional wakeup event is detected if the WKUP pin is
 * enabled
  *                  (by setting the EWUP bit) when the WKUP pin level is
 * already high.
 ...
 * enabled
  *                  by the PWR_PVDCmd() function. The PVD is stopped by Standby
 * mode
  *                  For this reason, this bit is equal to 0 after Standby or
 * reset
  *                  until the PVDE bit is set.
  *            @arg PWR_FLAG_BRR: Backup regulator ready flag. This bit is not
 * reset
  *                  when the device wakes up from Standby mode or by a system
 * reset
  *                  or power reset.
