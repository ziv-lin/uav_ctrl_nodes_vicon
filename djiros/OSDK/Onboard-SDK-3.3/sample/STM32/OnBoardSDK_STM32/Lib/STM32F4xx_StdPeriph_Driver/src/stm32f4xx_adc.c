     ...
     *** Injected channels group configuration ***
     ...
     [..]
       ...
       (+) To activate the AutoInjected mode, use the ADC_AutoInjectedConvCmd()
           function.
       (+) To read the ADC converted values, use the
...
for
      ...
      (+) Delay between 2 sampling phases (used in dual or triple interleaved
modes)
 ...
 * parameters
  ...
  *         Resolution and Data Alignment), however, the rest of the
 * configuration
...
ADC_Init(ADC_TypeDef* ADCx, ADC_InitTypeDef* ADC_InitStruct)
{
  ...
  tmpreg1 |= (uint32_t)(
    ADC_InitStruct->ADC_DataAlign | ADC_InitStruct->ADC_ExternalTrigConv |
...
/**
  * @brief  Initializes the ADCs peripherals according to the specified
 * parameters
  *         in the ADC_CommonInitStruct.
  * @param  ADC_CommonInitStruct: pointer to an ADC_CommonInitTypeDef structure
  *         that contains the configuration information for  All ADCs
 * peripherals.
  * @retval None
  */
void
...
@verbatim
 ...
 ===============================================================================
    [..] This section provides functions allowing to configure the Analog
Watchdog
...
the
      ...
      (#) The Analog watchdog is enabled and configured to enable the check, on
one
 ...
 * single regular channel
  *            @arg ADC_AnalogWatchdog_SingleInjecEnable: Analog watchdog on a
 * single injected channel
  *            @arg ADC_AnalogWatchdog_SingleRegOrInjecEnable: Analog watchdog
 * on a single regular or injected channel
  *            @arg ADC_AnalogWatchdog_AllRegEnable: Analog watchdog on all
 * regular channel
  *            @arg ADC_AnalogWatchdog_AllInjecEnable: Analog watchdog on all
 * injected channel
  *            @arg ADC_AnalogWatchdog_AllRegAllInjecEnable: Analog watchdog on
 * all regular and injected channels
  *            @arg ADC_AnalogWatchdog_None: No channel guarded by the analog
 * watchdog
  * @retval None
  */
void
...
/**
  ...
  */
void
ADC_AnalogWatchdogThresholdsConfig(ADC_TypeDef* ADCx, uint16_t HighThreshold,
...
@verbatim
 ...
 ===============================================================================
    [..] This section provides functions allowing to enable/ disable the
internal
         connections between the ADC and the Temperature Sensor, the Vrefint and
         the Vbat sources.

    [..] A typical configuration to get the Temperature sensor and Vrefint
channels
         voltages is done following these steps :
      (#) Enable the internal connection of Temperature sensor and Vrefint
sources
    ...
    [..] A typical configuration to get the VBAT channel voltage is done
following
         these steps :
      (#) Enable the internal connection of VBAT source with the ADC channel
using
...
@verbatim
 ...
 ===============================================================================
    ...
    [..] This section provides functions allowing to manage the ADC's regular
channels,
...
subsection
      ...
      (#) Get the conversion data: This subsection provides an important
function in
          the ADC peripheral since it returns the converted data of the current
          regular channel. When the Conversion value is read, the EOC Flag is
          automatically cleared.

          -@- For multi ADC mode, the last ADC1, ADC2 and ADC3 regular
conversions
              results data (in the selected multi mode) can be returned in the
same
 ...
 * channel.
  ...
  */
void
ADC_RegularChannelConfig(ADC_TypeDef* ADCx, uint8_t ADC_Channel, uint8_t Rank,
...
/**
  * @brief  Enables the selected ADC software start conversion of the regular
 * channels.
  * @param  ADCx: where x can be 1, 2 or 3 to select the ADC peripheral.
  * @retval None
  */
...
/**
  ...
  *         channel.
  * @param  ADCx: where x can be 1, 2 or 3 to select the ADC peripheral.
  * @param  Number: specifies the discontinuous mode regular channel count
 * value.
...
/**
  ...
  *           Data[15:0] : these bits contain alternatively the regular data of
 * ADC1, ADC3 and ADC2.
  *           Data[31:16]: these bits contain alternatively the regular data of
 * ADC2, ADC1 and ADC3.
  */
uint32_t
...
@verbatim
 ...
 ===============================================================================
    [..] This section provides functions allowing to configure the DMA for ADC
         ...
         register, it is useful to use DMA for conversion of more than one
regular
...
each
    ...
    [..] Depending on the "DMA disable selection for Independent ADC mode"
         configuration (using the ADC_DMARequestAfterLastTransferCmd()
function),
    ...
    [..] Depending on the "DMA disable selection for multi ADC mode"
configuration
         (using the void ADC_MultiModeDMARequestAfterLastTransferCmd()
function),
         at the end of the last DMA transfer, two possibilities are allowed:
        (+) No new DMA request is issued to the DMA controller (feature
DISABLED)
        (+) Requests can continue to be generated (feature ENABLED).
...
/**
  * @brief  Enables or disables the ADC DMA request after last transfer
 * (Single-ADC mode)
  * @param  ADCx: where x can be 1, 2 or 3 to select the ADC peripheral.
  * @param  NewState: new state of the selected ADC DMA request after last
 * transfer.
  *          This parameter can be: ENABLE or DISABLE.
  * @retval None
  */
...
/**
  * @brief  Enables or disables the ADC DMA request after last transfer in multi
 * ADC mode
  * @param  NewState: new state of the selected ADC DMA request after last
 * transfer.
  *          This parameter can be: ENABLE or DISABLE.
  * @note   if Enabled, DMA requests are issued as long as data are converted
 * and
  *         DMA mode for multi ADC mode (selected using ADC_CommonInit()
 * function
...
channels,
      ...
      (#) Configuration functions for Injected channels: This subsection
provides
 ...
 * channel.
  ...
  */
void
ADC_InjectedChannelConfig(ADC_TypeDef* ADCx, uint8_t ADC_Channel, uint8_t Rank,
                          uint8_t ADC_SampleTime)
{
 ...
 * injected conversion.
 *          This parameter can be one of the following values:
 *            @arg ADC_ExternalTrigInjecConv_T1_CC4: Timer1 capture compare4
 * selected
 *            @arg ADC_ExternalTrigInjecConv_T1_TRGO: Timer1 TRGO event selected
 *            @arg ADC_ExternalTrigInjecConv_T2_CC1: Timer2 capture compare1
 * selected
 *            @arg ADC_ExternalTrigInjecConv_T2_TRGO: Timer2 TRGO event selected
 *            @arg ADC_ExternalTrigInjecConv_T3_CC2: Timer3 capture compare2
 * selected
 *            @arg ADC_ExternalTrigInjecConv_T3_CC4: Timer3 capture compare4
 * selected
 *            @arg ADC_ExternalTrigInjecConv_T4_CC1: Timer4 capture compare1
 * selected
 *            @arg ADC_ExternalTrigInjecConv_T4_CC2: Timer4 capture compare2
 * selected
 *            @arg ADC_ExternalTrigInjecConv_T4_CC3: Timer4 capture compare3
 * selected
 *            @arg ADC_ExternalTrigInjecConv_T4_TRGO: Timer4 TRGO event selected
 *            @arg ADC_ExternalTrigInjecConv_T5_CC4: Timer5 capture compare4
 * selected
 *            @arg ADC_ExternalTrigInjecConv_T5_TRGO: Timer5 TRGO event selected
 *            @arg ADC_ExternalTrigInjecConv_T8_CC2: Timer8 capture compare2
 * selected
 *            @arg ADC_ExternalTrigInjecConv_T8_CC3: Timer8 capture compare3
 * selected
 *            @arg ADC_ExternalTrigInjecConv_T8_CC4: Timer8 capture compare4
 * selected
 *            @arg ADC_ExternalTrigInjecConv_Ext_IT15: External interrupt line
 * 15 event selected
 * @retval None
 */
void
...
/**
  * @brief  Configures the ADCx external trigger edge for injected channels
 * conversion.
  * @param  ADCx: where x can be 1, 2 or 3 to select the ADC peripheral.
 ...
 * disabled for
  *                                                     injected conversion
  *            @arg ADC_ExternalTrigInjecConvEdge_Rising: detection on rising
 * edge
  *            @arg ADC_ExternalTrigInjecConvEdge_Falling: detection on falling
 * edge
  *            @arg ADC_ExternalTrigInjecConvEdge_RisingFalling: detection on
 * both rising
  *                                                               and falling
 * edge
  * @retval None
  */
void
ADC_ExternalTrigInjectedConvEdgeConfig(ADC_TypeDef* ADCx,
...
@verbatim
 ...
 ===============================================================================
    ...
    [..] This section provides functions allowing to configure the ADC
Interrupts
  ...
  ======================================================
    [..]
      (+) Flags :
        ...
        (##) ADC_FLAG_JSTRT: Injected channel start ==> to indicate hardware
when
  ...
  *** General Flags and Interrupts for the ADC ***
  ================================================
    [..]
      (+)Flags :
        (##) ADC_FLAG_AWD: Analog watchdog ==> to indicate if the converted
voltage
