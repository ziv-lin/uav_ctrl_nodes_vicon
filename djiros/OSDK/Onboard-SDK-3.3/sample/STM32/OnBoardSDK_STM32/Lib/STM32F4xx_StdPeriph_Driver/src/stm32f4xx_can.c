...
@verbatim
 ...
 ===============================================================================
    [..] This section provides functions allowing to
      (+) Initialize the CAN peripherals : Prescaler, operating mode, the
maximum
...
/**
  ...
  * @param  CAN_InitStruct: pointer to a CAN_InitTypeDef structure which ill be
 * initialized.
  * @retval None
  */
void
...
/**
  * @brief  Enables or disables the CAN Time TriggerOperation communication
 * mode.
...
@verbatim
 ...
 ===============================================================================
    [..] This section provides functions allowing to
      (+) Initiate and transmit a CAN frame message (if there is an empty
mailbox).
...
@verbatim
 ...
 ===============================================================================
    [..] This section provides functions allowing to select the CAN Operation
modes
      (+) sleep mode
      (+) normal mode
      (+) initialization mode
...
/**
  ...
  *         This parameter can be one of @ref CAN_OperatingMode_TypeDef
 * enumeration.
...
@verbatim
 ...
 ===============================================================================
     ...
     [..] This section provides functions allowing to configure the CAN
Interrupts
    ...
    [..] The 15 flags can be divided on 4 groups:
      ...
      (+) Transmit Flags
        ...
        (++) CAN_FLAG_RQCP2  : Request completed MailBoxes 0, 1 and 2  Flags
                               Set when when the last request (transmit or
abort)
                               has been performed.
      ...
      (+) Receive Flags
        ...
        (++) CAN_FLAG_FMP1   : FIFO 0 and 1 Message Pending Flags
                               set to signal that messages are pending in the
receive
                               FIFO.
        ...
        (++) CAN_FLAG_FF1    : FIFO 0 and 1 Full Flags
                               set when three messages are stored in the
selected
                               FIFO.

        (++) CAN_FLAG_FOV0
        (++) CAN_FLAG_FOV1   : FIFO 0 and 1 Overrun Flags
                               set when a new message has been received and
passed
      ...
      (+) Operating Mode Flags
        ...
        (++) CAN_FLAG_WKU    : Wake up Flag
                               set to signal that a SOF bit has been detected
while
                               the CAN hardware was in Sleep mode.

        (++) CAN_FLAG_SLAK   : Sleep acknowledge Flag
                               Set to signal that the CAN has entered Sleep
Mode.

      (+) Error Flags

        (++) CAN_FLAG_EWG    : Error Warning Flag
                               Set when the warning limit has been reached
(Receive
                               Error Counter or Transmit Error Counter greater
than 96).
        ...
        (++) CAN_FLAG_BOF    : Bus-Off Flag
                               set when CAN enters the bus-off state. The
bus-off
                               state is entered on TEC overflow, greater than
255.
                               This Flag is cleared only by hardware.

        (++) CAN_FLAG_LEC    : Last error code Flag
                               set If a message has been transferred (reception
or
                               transmission) with error, and the error code is
hold.
    ...
    [..] The 14 interrupts can be divided on 4 groups:
      ...
      (+) Receive Interrupts
        ...
        (++) CAN_IT_FMP1    :  FIFO 0 and FIFO1 message pending Interrupts
                               ...
                               The corresponding interrupt pending bits are
cleared
                               only by hardware.
        ...
        (++) CAN_IT_FF1     :  FIFO 0 and FIFO1 full Interrupts
                               if enabled, these interrupt sources are pending
                               when three messages are stored in the selected
FIFO.
      ...
      (+) Operating Mode Interrupts
        ...
        (++) CAN_IT_WKU     :  Wake-up Interrupt
                               if enabled, this interrupt source is pending when
                               a SOF bit has been detected while the CAN
hardware
                               was in Sleep mode.
      ...
      (+) Error Interrupts
        ...
        (++) CAN_IT_BOF     :  Bus-off Interrupt
                               if enabled, this interrupt source is pending when
                               CAN enters the bus-off state. The bus-off state
is
    ...
    [..] Managing the CAN controller events :
      ...
      (#) In the Interrupt Mode it is advised to use the following functions:
        ...
        (++) CAN_ClearITPendingBit() : to clear the Interrupt pending Bit
            (corresponding Flag).
        -@@-  This function has no impact on CAN_IT_FMP0 and CAN_IT_FMP1
Interrupts
...
CAN_ClearITPendingBit(CAN_TypeDef* CANx, uint32_t CAN_IT)
{
  ...
  switch (CAN_IT)
  {
    ...
    case CAN_IT_EWG:
      ...
      /* @note the corresponding Flag is cleared by hardware depending on the
       * CAN Bus status*/
      break;
    case CAN_IT_EPV:
      /* Clear CAN_MSR_ERRI (rc_w1) */
    ...
    case CAN_IT_ERR:
      ...
      /* Clear CAN_MSR_ERRI (rc_w1) */
      CANx->MSR = CAN_MSR_ERRI;
      /* @note BOFF, EPVF and EWGF Flags are cleared by hardware depending on
       * the CAN Bus status*/
      break;
    default:
      break;
