...
@verbatim
 ...
 [..]
   ...
   (#) Wait until the 32 bit Random number Generator contains a valid  random
data
...
/**
  * @brief  De-initializes the RNG peripheral registers to their default reset
 * values.
  * @param  None
  * @retval None
  */
...
/**
  ...
  * @note   Each time the the Random number data is read (using
 * RNG_GetRandomNumber()
 ...
 * provide
  *         the RNG clock and clear the CEIS bit (using
...
@verbatim
 ...
 [..] This section provides functions allowing to configure the RNG Interrupts
and
 ...
 *** Interrupts ***
 ...
 [..] If enabled, an RNG interrupt is pending :
   ...
   (#) In the case of a clock error : the PLL48CLK (RNG peripheral clock source)
       was not correctly detected (fPLL48CLK< fHCLK/16). This interrupt source
is
...
manage
   ...
   (#) In the Polling Mode it is advised to use the following functions:
       ...
       -@@- RNG_FLAG_DRDY can not be cleared by RNG_ClearFlag(). it is cleared
only
