/**
  ...
  *          ===================================================================
  *
  *            This section provide functions allowing to configure the NVIC
  interrupts (IRQ).
  *            The Cortex-M4 exceptions are managed by CMSIS functions.
  *
  *            1. Configure the NVIC Priority Grouping using
 ...
 * subpriority.
  ...
  *                                0 bits for subpriority
  * @note   When the NVIC_PriorityGroup_0 is selected, IRQ pre-emption is no
 * more possible.
  *         The pending IRQ priority will be managed only by the subpriority.
  * @retval None
  */
...
NVIC_Init(NVIC_InitTypeDef* NVIC_InitStruct)
{
  ...
  if (NVIC_InitStruct->NVIC_IRQChannelCmd != DISABLE)
  {
    ...
    tmppriority |=
      (uint8_t)(NVIC_InitStruct->NVIC_IRQChannelSubPriority & tmpsub);
 ...
 * memory.
  ...
  * @param  Offset: Vector Table base offset field. This value must be a
 * multiple of 0x200.
  * @retval None
  */
void
 ...
 * power mode.
  ...
  * @param  NewState: new state of LP condition. This parameter can be: ENABLE
 * or DISABLE.
  * @retval None
  */
void
