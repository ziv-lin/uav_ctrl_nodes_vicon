...
{
  uint32_t
    HASH_AlgoSelection;      /*!< SHA-1, SHA-224, SHA-256 or MD5. This parameter
  ...
  uint32_t HASH_DataType;    /*!< 32-bit data, 16-bit data, 8-bit data or
                                  ...
                                  @ref HASH_Data_Type */
  uint32_t HASH_HMACKeyType; /*!< HMAC Short key or HMAC Long Key. This
                                parameter
...
#define HASH_AlgoSelection_SHA1                                                \
  ((uint32_t)0x0000) /*!< HASH function is SHA1   */
...
#define IS_HASH_ALGOMODE(ALGOMODE)                                             \
  (((ALGOMODE) == HASH_AlgoMode_HASH) || ((ALGOMODE) == HASH_AlgoMode_HMAC))
/**
  * @}
  */
...
#define HASH_DataType_32b                                                      \
  ((uint32_t)0x0000) /*!< 32-bit data. No swapping                     */
#define HASH_DataType_16b                                                      \
  HASH_CR_DATATYPE_0 /*!< 16-bit data. Each half word is swapped       */
#define HASH_DataType_8b                                                       \
  HASH_CR_DATATYPE_1 /*!< 8-bit data. All bytes are swapped            */
#define HASH_DataType_1b                                                       \
  HASH_CR_DATATYPE /*!< 1-bit data. In the word all bits are swapped */

#define IS_HASH_DATATYPE(DATATYPE)                                             \
  (((DATATYPE) == HASH_DataType_32b) || ((DATATYPE) == HASH_DataType_16b) ||   \
   ((DATATYPE) == HASH_DataType_8b) || ((DATATYPE) == HASH_DataType_1b))
/**
  * @}
  */
...
#define HASH_HMACKeyType_ShortKey                                              \
  ((uint32_t)0x00000000)                      /*!< HMAC Key is <= 64 bytes */
...
/** @defgroup HASH_interrupts_definition
  ...
  */
#define HASH_IT_DINI                                                           \
  HASH_IMR_DINIM /*!< A new block can be entered into the input buffer (DIN)   \
                    */
#define HASH_IT_DCI                                                            \
  HASH_IMR_DCIM /*!< Digest calculation complete                            */

#define IS_HASH_IT(IT)                                                         \
  ((((IT) & (uint32_t)0xFFFFFFFC) == 0x00000000) && ((IT) != 0x00000000))
...
#define HASH_FLAG_DINIS                                                        \
  HASH_SR_DINIS /*!< 16 locations are free in the DIN : A new block can be     \
...
#define HASH_FLAG_DINNE                                                        \
  HASH_CR_DINNE /*!< DIN not empty : The input buffer contains at least one    \
                   word of data                 */

#define IS_HASH_GET_FLAG(FLAG)                                                 \
  (((FLAG) == HASH_FLAG_DINIS) || ((FLAG) == HASH_FLAG_DCIS) ||                \
