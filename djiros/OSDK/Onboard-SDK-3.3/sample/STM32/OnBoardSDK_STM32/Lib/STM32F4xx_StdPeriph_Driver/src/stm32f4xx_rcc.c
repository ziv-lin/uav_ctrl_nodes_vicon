...
#define DCKCFGR_TIMPRE_BB                                                      \
  (PERIPH_BB_BASE + (DCKCFGR_OFFSET * 32) + (TIMPRE_BitNumber * 4))
...
/** @defgroup RCC_Group1 Internal and external clocks, PLL, CSS and MCO
configuration functions
 *  @brief   Internal and external clocks, PLL, CSS and MCO configuration
functions
 *
@verbatim
 ===================================================================================
...
clocks,
      ...
      (#) HSI (high-speed internal), 16 MHz factory-trimmed RC used directly or
through
...
(48 MHz),
      ...
      (#) PLLI2S (clocked by HSI or HSE), used to generate an accurate clock to
achieve
...
clock
         is automatically switched to HSI and an interrupt is generated if
enabled.
...
PLL
      ...
      (#) MCO2 (microcontroller clock output), used to output HSE, PLL, SYSCLK
or PLLI2S
 ...
 * system(CSS)
  *         was previously enabled you have to enable it again after calling
 * this
  *         function.
...
/**
  ...
  *         and this flag is not set. The timeout value is defined by the
 * constant
 ...
 * HSIRDY
  *         flag to be set indicating that HSI clock is stable and can be used
 * as
  *         system clock source.
...
/**
  * @brief  Configures the main PLL clock source, multiplication and division
 * factors.
 ...
 * frequency
  ...
  * @note   You have to set the PLLP parameter correctly to not exceed 168 MHz
 * on
 ...
 * modes.
  * @param  NewState: new state of the main PLL. This parameter can be: ENABLE
 * or DISABLE.
  * @retval None
  */
void
...
/**
  ...
  *
  * @param  PLLI2SN: specifies the multiplication factor for PLLI2S VCO output
 * clock
  *          This parameter must be a number between 192 and 432.
 ...
 * VCO
  ...
  * @note   You have to set the PLLI2SR parameter correctly to not exceed 192
 * MHz
 ...
 * modes.
  * @param  NewState: new state of the PLLI2S. This parameter can be: ENABLE or
 * DISABLE.
  * @retval None
  */
void
...
/**
  ...
  * @note   If a failure is detected on the HSE oscillator clock, this
 * oscillator
  *         is automatically disabled and an interrupt is generated to inform
 * the
...
/**
  ...
  *            @arg RCC_MCO1Source_PLLCLK: main PLL clock selected as MCO1
 * source
...
/**
  ...
  *          This parameter can be one of the following values:
  *            @arg RCC_MCO2Source_SYSCLK: System clock (SYSCLK) selected as
 * MCO2 source
  *            @arg RCC_MCO2Source_PLLI2SCLK: PLLI2S clock selected as MCO2
 * source
  *            @arg RCC_MCO2Source_HSE: HSE clock selected as MCO2 source
  *            @arg RCC_MCO2Source_PLLCLK: main PLL clock selected as MCO2
 * source
...
/** @defgroup RCC_Group2 System AHB and APB busses clocks configuration
functions
...
@verbatim
 ...
 ===============================================================================
    [..]
      This section provide functions allowing to configure the System, AHB, APB1
and
      APB2 busses clocks.
...
HSI,
          ...
          on AHB bus (DMA, GPIO...). APB1 (PCLK1) and APB2 (PCLK2) clocks are
derived
...
clocks.
      ...
      -@- All the peripheral clocks are derived from the System clock (SYSCLK)
        ...
        (+@) I2S: the I2S clock can be derived either from a specific PLL
(PLLI2S) or
             from an external clock mapped on the I2S_CKIN pin.
             You have to use RCC_I2SCLKConfig() function to configure this
clock.
        (+@) RTC: the RTC clock can be derived either from the LSI, LSE or HSE
clock
             divided by 2 to 31. You have to use RCC_RTCCLKConfig() and
RCC_RTCCLKCmd()
             functions to configure this clock.
        (+@) USB OTG FS, SDIO and RTC: USB OTG FS require a frequency equal to
48 MHz
             to work correctly, while the SDIO require a frequency equal or
lower than
 ...
 | |---------------------------------------------------------------------|
 |               | voltage range  | voltage range  | voltage range   | voltage
range   |
 |               | 2.7 V - 3.6 V  | 2.4 V - 2.7 V  | 2.1 V - 2.4 V   | 1.8 V -
2.1 V   |
 |---------------|----------------|----------------|-----------------|-----------------|
 |0WS(1CPU cycle)|0 < HCLK <= 30  |0 < HCLK <= 24  |0 < HCLK <= 18   |0 < HCLK
<= 16   |
 |---------------|----------------|----------------|-----------------|-----------------|
 |1WS(2CPU cycle)|30 < HCLK <= 60 |24 < HCLK <= 48 |18 < HCLK <= 36  |16 < HCLK
<= 32  |
 |---------------|----------------|----------------|-----------------|-----------------|
 |2WS(3CPU cycle)|60 < HCLK <= 90 |48 < HCLK <= 72 |36 < HCLK <= 54  |32 < HCLK
<= 48  |
 |---------------|----------------|----------------|-----------------|-----------------|
 |3WS(4CPU cycle)|90 < HCLK <= 120|72 < HCLK <= 96 |54 < HCLK <= 72  |48 < HCLK
<= 64  |
 |---------------|----------------|----------------|-----------------|-----------------|
 |4WS(5CPU cycle)|120< HCLK <= 150|96 < HCLK <= 120|72 < HCLK <= 90  |64 < HCLK
<= 80  |
 |---------------|----------------|----------------|-----------------|-----------------|
 |5WS(6CPU cycle)|120< HCLK <= 168|120< HCLK <= 144|90 < HCLK <= 108 |80 < HCLK
<= 96  |
 |---------------|----------------|----------------|-----------------|-----------------|
 |6WS(7CPU cycle)|      NA        |144< HCLK <= 168|108 < HCLK <= 120|96 < HCLK
<= 112 |
 |---------------|----------------|----------------|-----------------|-----------------|
 |7WS(8CPU cycle)|      NA        |      NA        |120 < HCLK <= 138|112 < HCLK
<= 120|
 +-------------------------------------------------------------------------------------+
      -@- When VOS bits (in PWR_CR register) is reset to 0 , the maximum value
of HCLK is 144 MHz.
          You can use PWR_MainRegulatorModeConfig() function to set or reset
this bit.
 ...
 * correctly
  *         these bits to ensure that HCLK not exceed the maximum allowed
 * frequency
 ...
 * HSE_VALUE(**)
  ...
  * @note     (*) HSI_VALUE is a constant defined in stm32f4xx.h file (default
 * value
  *               16 MHz) but the real value may vary depending on the
 * variations
  *               in voltage and temperature.
  * @note     (**) HSE_VALUE is a constant defined in stm32f4xx.h file (default
 * value
  *                25 MHz), user has to ensure that HSE_VALUE is same as the
 * real
 ...
 * parameters.
  * @note   Each time SYSCLK, HCLK, PCLK1 and/or PCLK2 clock changes, this
 * function
...
@verbatim
 ...
 ===============================================================================
    [..] This section provide functions allowing to configure the Peripheral
clocks.
...
off
          except internal SRAM, Flash and JTAG. Before to start using a
peripheral
          you have to enable its interface clock. You can do this using
          RCC_AHBPeriphClockCmd(), RCC_APB2PeriphClockCmd() and
 ...
 * write
  *         access is denied to this domain after reset, you have to enable
 * write
  *         access using PWR_BackupAccessCmd(ENABLE) function before to
 * configure
...
RCC_RTCCLKConfig(uint32_t RCC_RTCCLKSource)
{
  ...
  { /* If HSE is selected as RTC clock source, configure HSE division factor for
       RTC clock */
    tmpreg = RCC->CFGR;
...
/**
  ...
  * @note   This function must be used only after the RTC clock source was
 * selected
...
/**
  ...
  *          This parameter can be one of the following values:
  *            @arg RCC_I2S2CLKSource_PLLI2S: PLLI2S clock used as I2S clock
 * source
  *            @arg RCC_I2S2CLKSource_Ext: External clock mapped on the I2S_CKIN
 * pin
...
/**
  ...
  * @param  RCC_TIMCLKPrescaler : specifies the Timers clocks prescalers
 * selection
  *         This parameter can be one of the following values:
  *            @arg RCC_TIMPrescDesactivated: The Timers kernels clocks
 * prescaler is
  *                 equal to HPRE if PPREx is corresponding to division by 1 or
 * 2,
  *                 else it is equal to [(HPRE * PPREx) / 2] if PPREx is
 * corresponding to
  *                 division by 4 or more.
  *
  *            @arg RCC_TIMPrescActivated: The Timers kernels clocks prescaler
 * is
  *                 equal to HPRE if PPREx is corresponding to division by 1, 2
 * or 4,
  *                 else it is equal to [(HPRE * PPREx) / 4] if PPREx is
 * corresponding
  *                 to division by 8 or more.
  * @retval None
  */
void
RCC_TIMCLKPresConfig(uint32_t RCC_TIMCLKPrescaler)
{
  /* Check the parameters */
...
/**
  * @brief  Enables or disables the AHB1 peripheral clock during Low Power
 * (Sleep) mode.
...
/**
  * @brief  Enables or disables the AHB2 peripheral clock during Low Power
 * (Sleep) mode.
  * @note   Peripheral clock gating in SLEEP mode can be used to further reduce
  *           power consumption.
  * @note   After wakeup from SLEEP mode, the peripheral clock is enabled again.
...
/**
  * @brief  Enables or disables the AHB3 peripheral clock during Low Power
 * (Sleep) mode.
  * @note   Peripheral clock gating in SLEEP mode can be used to further reduce
  *         power consumption.
  * @note   After wakeup from SLEEP mode, the peripheral clock is enabled again.
...
/**
  * @brief  Enables or disables the APB1 peripheral clock during Low Power
 * (Sleep) mode.
  * @note   Peripheral clock gating in SLEEP mode can be used to further reduce
  *         power consumption.
  * @note   After wakeup from SLEEP mode, the peripheral clock is enabled again.
...
/**
  * @brief  Enables or disables the APB2 peripheral clock during Low Power
 * (Sleep) mode.
  * @note   Peripheral clock gating in SLEEP mode can be used to further reduce
  *         power consumption.
  * @note   After wakeup from SLEEP mode, the peripheral clock is enabled again.
...
RCC_ITConfig(uint8_t RCC_IT, FunctionalState NewState)
{
  ...
  if (NewState != DISABLE)
  {
    /* Perform Byte access to RCC_CIR[14:8] bits to enable the selected
     * interrupts */
    *(__IO uint8_t*)CIR_BYTE2_ADDRESS |= RCC_IT;
  }
  else
  {
    /* Perform Byte access to RCC_CIR[14:8] bits to disable the selected
     * interrupts */
