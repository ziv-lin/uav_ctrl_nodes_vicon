 ...
 * correctly
  * released on the I2C bus (the bus is free, no other devices is
 * communicating).
  *
  */
/* --EV5 */
 ...
 * communication:
  * Master transmitter or Receiver). Then the master has to wait that a slave
 * acknowledges
  * his address. If an acknowledge is sent on the bus, one of the following
 * events will
  * be set:
  *
 ...
 * START
  *  and checking on EV5) has to send the header of 10-bit addressing mode
 * (I2C_SendData()
  *  function). Then master should wait on EV9. It means that the 10-bit
 * addressing
  *  header has been correctly sent on the bus. Then master should send the
 * second part of
  *  the 10-bit address (LSB) using the function I2C_Send7bitAddress(). Then
 * master
 ...
 * address
  * acknowledged) then the master has to check on one of the following events
 * for
 ...
 * transmission
  ...
  *        managed before the current byte end of transfer, then user may check
 * on EV7
...
/**
  ...
  * the I2C peripheral detected a Start condition on the bus (generated by
 * master
 ...
 * by
  *   I2C_OwnAddress1 field) the I2C_EVENT_SLAVE_XXX_ADDRESS_MATCHED event is
 * set
 ...
 * is expecting
  *      the end of the byte transmission. The two events
 * I2C_EVENT_SLAVE_BYTE_TRANSMITTED and
  *      I2C_EVENT_SLAVE_BYTE_TRANSMITTING are similar. The second one can
 * optionally be
  *      used when the user software doesn't guarantee the EV3 is managed before
 * the
  *      current byte end of transfer.
  *    - EV3_2: When the master sends a NACK in order to tell slave that data
 * transmission
  *      shall end (before sending the STOP condition). In this case slave has
 * to stop sending
