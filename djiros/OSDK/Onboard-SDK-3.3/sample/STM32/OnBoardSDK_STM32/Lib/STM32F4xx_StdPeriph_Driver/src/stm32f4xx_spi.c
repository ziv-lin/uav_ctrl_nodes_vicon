...
@verbatim
 ...
 [..]
   ...
   (#) Peripherals alternate function:
       ...
       (++) Call GPIO_Init() function In I2S mode, if an external clock source
is
            used then the I2S CKIN pin should be also configured in Alternate
            function Push-pull pull-up mode.

   (#) Program the Polarity, Phase, First Data, Baud Rate Prescaler, Slave
       Management, Peripheral Mode and CRC Polynomial values using the
SPI_Init()
       function.
       ...
       (++) I2S PLL is configured using the functions
            RCC_I2SCLKConfig(RCC_I2S2CLKSource_PLLI2S), RCC_PLLI2SCmd(ENABLE)
and
       ...
       (++) External clock source is configured using the function
            ...
            the define constant I2S_EXTERNAL_CLOCK_VAL in the stm32f4xx_conf.h
file.
   ...
   (#) Optionally, you can enable/configure the following parameters without
       re-initialization (i.e there is no need to call again SPI_Init()
function):
       (++) When bidirectional mode (SPI_Direction_1Line_Rx or
SPI_Direction_1Line_Tx)
            is programmed as Data direction parameter using the SPI_Init()
function
            it can be possible to switch between SPI_Direction_Tx or
SPI_Direction_Rx
            using the SPI_BiDirectionalLineConfig() function.
       (++) When SPI_NSS_Soft is selected as Slave Select Management parameter
            using the SPI_Init() function it can be possible to manage the
            NSS internal signal using the SPI_NSSInternalSoftwareConfig()
function.
 ...
 [..] It is possible to use SPI in I2S full duplex mode, in this case, each SPI
      ...
      using two data lines. Each SPI peripheral has an extended block called
I2Sxext
      (ie. I2S2ext for SPI2 and I2S3ext for SPI3).
      The extension block is not a full SPI IP, it is used only as I2S slave to
      implement full duplex mode. The extension block uses the same clock
sources
      as its master.
      ...
      (#) Call the I2S_FullDuplexConfig() function using the same strucutre
passed to
          I2S_Init() function.
 ...
 [..] Functions that can be called with I2Sxext instances are: I2S_Cmd(),
      ...
      SPI_I2S_ClearFlag(), SPI_I2S_GetITStatus() and
SPI_I2S_ClearITPendingBit().

      Example: To use SPI3 in Full duplex mode (SPI3 is Master Tx, I2S3ext is
Slave Rx):
...
Master
      mode and Slave mode (details for these procedures are available in
reference
      manual (RM0090)).
...
/**
  * @brief  De-initialize the SPIx peripheral registers to their default reset
 * values.
  * @param  SPIx: To select the SPIx/I2Sx peripheral, where x can be: 1, 2, 3,
 * 4, 5 or 6
  *         in SPI mode or 2 or 3 in I2S mode.
  *
  * @note   The extended I2S blocks (ie. I2S2ext and I2S3ext blocks) are
 * de-initialized
  *         when the relative I2S peripheral is de-initialized (the extended
 * block's clock
...
/**
  ...
  * @param  SPIx: where x can be 1, 2, 3, 4, 5 or 6 to select the SPI
 * peripheral.
 ...
 * peripheral
  *         configured in I2S mode.
  *
  * @note   The function calculates the optimal prescaler needed to obtain the
 * most
  *         accurate audio frequency (depending on the I2S clock source, the PLL
 * values
  *         and the product configuration). But in case the prescaler value is
 * greater
  *         than 511, the default value (0x02) will be configured instead.
  *
  * @note   if an external clock is used as source clock for the I2S, then the
 * define
  *         I2S_EXTERNAL_CLOCK_VAL in file stm32f4xx_conf.h should be enabled
 * and set
...
/* If an external I2S clock has to be used, this define should be set
   in the project configuration or in the stm32f4xx_conf.h file */
    ...
    else
    {
      ...
      tmp = (uint16_t)(((((i2sclk / (32 * packetlength)) * 10) /
...
/**
  * @brief  Selects the data transfer direction in bidirectional mode for the
 * specified SPI.
  * @param  SPIx: where x can be 1, 2, 3, 4, 5 or 6 to select the SPI
 * peripheral.
...
/**
  ...
  *         I2S_InitStruct.
  * @param  I2Sxext: where x can be  2 or 3 to select the I2S peripheral
 * extension block.
  * @param  I2S_InitStruct: pointer to an I2S_InitTypeDef structure that
 ...
 * peripheral
  ...
  *         configured as transmitter, the slave will be receiver and vice
 * versa.
  *         Or you can force a different mode by modifying the field I2S_Mode to
 * the
  *         value I2S_SlaveRx or I2S_SlaveTx indepedently of the master
 * configuration.
  *
...
@verbatim
 ...
 [..] This section provides a set of functions allowing to manage the SPI data
      transfers. In reception, data are received and then stored into an
internal
      Rx buffer while. In transmission, data are first stored into an internal
Tx
      buffer before being transmitted.

 [..] The read access of the SPI_DR register can be done using the
SPI_I2S_ReceiveData()
      function and returns the Rx buffered value. Whereas a write access to the
SPI_DR
      can be done using SPI_I2S_SendData() function and stores the written data
into
      Tx buffer.
...
/**
  ...
  * @param  SPIx: To select the SPIx/I2Sx peripheral, where x can be: 1, 2, 3,
 * 4, 5 or 6
  *         in SPI mode or 2 or 3 in I2S mode or I2Sxext for I2S full duplex
 * mode.
...
Prescaler,
       Slave Management, Peripheral Mode and CRC Polynomial values using the
SPI_Init()
       function.
   (#) Enable the CRC calculation using the SPI_CalculateCRC() function.
   (#) Enable the SPI using the SPI_Cmd() function
   (#) Before writing the last data to the TX buffer, set the CRCNext bit using
the
       SPI_TransmitCRC() function to indicate that after transmission of the
last
       data, the CRC should be transmitted.
   (#) After transmitting the last data, the SPI transmits the CRC. The
SPI_CR1_CRCNEXT
        bit is reset. The CRC is also received and compared against the
SPI_RXCRCR
        value.
...
interrupt
 ...
 [..]
   (@) It is advised not to read the calculated CRC values during the
communication.

   (@) When the SPI is in slave mode, be careful to enable CRC calculation only
       when the clock is stable, that is, when the clock is in the steady state.
       If not, a wrong CRC calculation may be done. In fact, the CRC is
sensitive
   ...
   (@) With high bitrate frequencies, be careful when transmitting the CRC.
       ...
       In fact, CRCNEXT bit has to be written before the end of the
transmission/reception
       of the last data.

   (@) For high bit rate frequencies, it is advised to use the DMA mode to avoid
the
...
CRC
       ...
       This may happen for example in case of a multi-slave environment where
the
       communication master addresses slaves alternately.

   (@) Between a slave de-selection (high level on NSS) and a new slave
selection
 ...
 * 4, 5 or 6
  *         in SPI mode or 2 or 3 in I2S mode or I2Sxext for I2S full duplex
 * mode.
  * @param  SPI_I2S_DMAReq: specifies the SPI DMA transfer request to be enabled
 * or disabled.
...
[..] In Polling Mode, the SPI/I2S communication can be managed by 9 flags:
  ...
  (#) SPI_I2S_FLAG_BSY : to indicate the state of the communication layer of the
SPI.
  ...
  (@) Do not use the BSY flag to handle each data transmission or reception. It
is
      better to use the TXE and RXNE flags instead.

 [..] In this Mode it is advised to use the following functions:
   (+) FlagStatus SPI_I2S_GetFlagStatus(SPI_TypeDef* SPIx, uint16_t
SPI_I2S_FLAG);
...
sources
      and 7 pending bits:
   (+) Pending Bits:
       (##) SPI_I2S_IT_TXE : to indicate the status of the transmit buffer
register
       (##) SPI_I2S_IT_RXNE : to indicate the status of the receive buffer
register
       (##) SPI_IT_CRCERR : to indicate if a CRC Calculation error occur
(available in SPI mode only)
       (##) SPI_IT_MODF : to indicate if a Mode Fault error occur (available in
SPI mode only)
       (##) SPI_I2S_IT_OVR : to indicate if an Overrun error occur
       (##) I2S_IT_UDR : to indicate an Underrun Error occurs (available in I2S
mode only).
       (##) I2S_FLAG_TIFRFE : to indicate a Frame Format error occurs (available
in TI mode only).

   (+) Interrupt Source:
       (##) SPI_I2S_IT_TXE: specifies the interrupt source for the Tx buffer
empty
            interrupt.
       (##) SPI_I2S_IT_RXNE : specifies the interrupt source for the Rx buffer
not
            empty interrupt.
       (##) SPI_I2S_IT_ERR : specifies the interrupt source for the errors
interrupt.
 ...
 * 4, 5 or 6
  *         in SPI mode or 2 or 3 in I2S mode or I2Sxext for I2S full duplex
 * mode.
  * @param  SPI_I2S_IT: specifies the SPI interrupt source to be enabled or
 * disabled.
  *          This parameter can be one of the following values:
 ...
 * mode.
  ...
  *          operation to SPI_DR register (SPI_I2S_ReceiveData()) followed by a
 * read
...
/**
  * @brief  Checks whether the specified SPIx/I2Sx interrupt has occurred or
 * not.
  * @param  SPIx: To select the SPIx/I2Sx peripheral, where x can be: 1, 2, 3,
 * 4, 5 or 6
  *         in SPI mode or 2 or 3 in I2S mode or I2Sxext for I2S full duplex
 ...
 * (SPI_I2S_GetITStatus()).
  ...
  * @note   MODF (Mode Fault) interrupt pending bit is cleared by software
 * sequence:
  *          a read/write operation to SPI_SR register (SPI_I2S_GetITStatus())
  *          followed by a write operation to SPI_CR1 register (SPI_Cmd() to
 * enable
  *          the SPI).
  * @retval None
  */
