...
@verbatim
 ...
 [..]
   (#) The SDIO clock (SDIOCLK = 48 MHz) is coming from a specific output of PLL
       (PLL48CLK). Before to start working with SDIO peripheral make sure that
the
...
ENABLE).
   ...
   (#) Enable the Power ON State using the
SDIO_SetPowerState(SDIO_PowerState_ON)
   ...
   (#) To control the CPSM (Command Path State Machine) and send
       ...
       SDIO_GetCommandResponse() and SDIO_GetResponse() functions. First, user
has
 ...
 * disabled.
  ...
  *            @arg SDIO_IT_CCRCFAIL: Command response received (CRC check
 * failed) interrupt
  *            @arg SDIO_IT_DCRCFAIL: Data block sent/received (CRC check
 * failed) interrupt
  *            @arg SDIO_IT_CTIMEOUT: Command response timeout interrupt
  *            @arg SDIO_IT_DTIMEOUT: Data timeout interrupt
  *            @arg SDIO_IT_TXUNDERR: Transmit FIFO underrun error interrupt
  *            @arg SDIO_IT_RXOVERR:  Received FIFO overrun error interrupt
  *            @arg SDIO_IT_CMDREND:  Command response received (CRC check
 * passed) interrupt
  *            @arg SDIO_IT_CMDSENT:  Command sent (no response required)
 * interrupt
  *            @arg SDIO_IT_DATAEND:  Data end (data counter, SDIDCOUNT, is
 * zero) interrupt
  *            @arg SDIO_IT_STBITERR: Start bit not detected on all data signals
 * in wide
  *                                   bus mode interrupt
  *            @arg SDIO_IT_DBCKEND:  Data block sent/received (CRC check
 * passed) interrupt
  ...
  *            @arg SDIO_IT_CEATAEND: CE-ATA command completion signal received
 * for CMD61 interrupt
...
/**
  ...
  *          This parameter can be one of the following values:
  *            @arg SDIO_FLAG_CCRCFAIL: Command response received (CRC check
 * failed)
  *            @arg SDIO_FLAG_DCRCFAIL: Data block sent/received (CRC check
 * failed)
 ...
 * zero)
  *            @arg SDIO_FLAG_STBITERR: Start bit not detected on all data
 * signals in wide bus mode.
  *            @arg SDIO_FLAG_DBCKEND:  Data block sent/received (CRC check
 * passed)
  ...
  *            @arg SDIO_FLAG_CEATAEND: CE-ATA command completion signal
 * received for CMD61
 ...
 * passed) interrupt
  *            @arg SDIO_IT_CMDSENT:  Command sent (no response required)
 * interrupt
  *            @arg SDIO_IT_DATAEND:  Data end (data counter, SDIO_DCOUNT, is
 * zero) interrupt
  *            @arg SDIO_IT_STBITERR: Start bit not detected on all data signals
 * in wide
  *                                   bus mode interrupt
