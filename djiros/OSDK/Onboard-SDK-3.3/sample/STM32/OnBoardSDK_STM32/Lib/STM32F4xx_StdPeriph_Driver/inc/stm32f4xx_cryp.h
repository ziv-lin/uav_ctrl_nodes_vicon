...
typedef struct
{
  ...
  uint32_t CRYP_AlgoMode; /*!< TDES-ECB, TDES-CBC, DES-ECB, DES-CBC, AES-ECB,
                             ...
                             CRYP_Algorithm_Mode */
  uint32_t
    CRYP_DataType;       /*!< 32-bit data, 16-bit data, bit data or bit string.
...
#define IS_CRYP_ALGODIR(ALGODIR)                                               \
  (((ALGODIR) == CRYP_AlgoDir_Encrypt) || ((ALGODIR) == CRYP_AlgoDir_Decrypt))
...
#define IS_CRYP_PHASE(PHASE)                                                   \
  (((PHASE) == CRYP_Phase_Init) || ((PHASE) == CRYP_Phase_Header) ||           \
   ((PHASE) == CRYP_Phase_Payload) || ((PHASE) == CRYP_Phase_Final))
...
#define IS_CRYP_KEYSIZE(KEYSIZE)                                               \
  (((KEYSIZE) == CRYP_KeySize_128b) || ((KEYSIZE) == CRYP_KeySize_192b) ||     \
...
#define CRYP_FLAG_BUSY                                                         \
  ((uint8_t)0x10)                       /*!< The CRYP core is currently        \
...
#define CRYP_FLAG_OUTRIS                                                       \
  ((uint8_t)0x21) /*!< Output Fifo service raw                                 \
                       interrupt status */

#define IS_CRYP_GET_FLAG(FLAG)                                                 \
  (((FLAG) == CRYP_FLAG_IFEM) || ((FLAG) == CRYP_FLAG_IFNF) ||                 \
...
ErrorStatus CRYP_AES_GCM(uint8_t Mode, uint8_t InitVectors[16], uint8_t* Key,
                         uint16_t Keysize, uint8_t* Input, uint32_t ILength,
                         uint8_t* Header, uint32_t HLength, uint8_t* Output,
                         uint8_t* AuthTAG);

ErrorStatus CRYP_AES_CCM(uint8_t Mode, uint8_t* Nonce, uint32_t NonceSize,
                         uint8_t* Key, uint16_t Keysize, uint8_t* Input,
                         uint32_t ILength, uint8_t* Header, uint32_t HLength,
                         uint8_t* HBuffer, uint8_t* Output, uint8_t* AuthTAG,
                         uint32_t TAGSize);
