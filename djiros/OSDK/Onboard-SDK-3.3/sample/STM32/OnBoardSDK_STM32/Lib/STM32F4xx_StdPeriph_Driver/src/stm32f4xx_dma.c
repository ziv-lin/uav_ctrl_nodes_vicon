/**
 ...
 RCC_AHB1PeriphResetCmd(RCC_AHB1Periph_DMA1, ENABLE)
          function for DMA1 or using RCC_AHB1PeriphResetCmd(RCC_AHB1Periph_DMA2,
 ENABLE)
          function for DMA2.
 ...
 Source and Destination
          data formats, Circular or Normal mode, Stream Priority level, Source
 and Destination
          Incrementation mode, FIFO mode and its Threshold (if needed), Burst
          mode for Source and/or Destination (if needed) using the DMA_Init()
 function.
          To avoid filling unneccessary fields, you can call DMA_StructInit()
 function
          to initialize a given structure with default values (reset values),
 the modify
          only necessary fields
          (ie. Source and Destination addresses, Transfer size and Data
 Formats).

      (#) Enable the NVIC and the corresponding interrupt(s) using the function
          DMA_ITConfig() if you need to use DMA interrupts.

      (#) Optionally, if the Circular mode is enabled, you can use the Double
 buffer mode by configuring
          the second Memory address and the first Memory to be used through the
 function
          DMA_DoubleBufferModeConfig(). Then enable the Double buffer mode
 through the function
          DMA_DoubleBufferModeCmd(). These operations must be done before step
 6.
      ...
      (#) Activate the needed Stream Request using PPP_DMACmd() function for
          ...
          The function allowing this operation is provided in each PPP
 peripheral
          driver (ie. SPI_DMACmd for SPI peripheral).
          Once the Stream is enabled, it is not possible to modify its
 configuration
          unless the stream is stopped and disabled.
          After enabling the Stream, it is advised to monitor the EN bit status
 using
          the function DMA_GetCmdStatus(). In case of configuration errors or
 bus errors
          this bit will remain reset and all transfers on this Stream will
 remain on hold.

      (#) Optionally, you can configure the number of data to be transferred
          when the Stream is disabled (ie. after each Transfer Complete event
          or when a Transfer Error occurs) using the function
 DMA_SetCurrDataCounter().
          And you can get the number of remaining data to be transferred using
          the function DMA_GetCurrDataCounter() at run time (when the DMA Stream
 is
          enabled and running).

      (#) To control DMA events you can use one of the following two methods:
        (##) Check on DMA Stream flags using the function DMA_GetFlagStatus().
        (##) Use DMA interrupts through the function DMA_ITConfig() at
 initialization
    ...
    [..]
      ...
      (#) Optionally, if Circular mode and Double Buffer mode are enabled, you
 can modify
          the Memory Addresses using the function DMA_MemoryTargetConfig(). Make
 sure that
          the Memory Address to be modified is not the one currently in use by
 DMA Stream.
          This condition can be monitored using the function
 DMA_GetCurrentMemoryTarget().
      ...
      (#) Optionally, Pause-Resume operations may be performed:
          ...
          Stream will cause the transfer to be paused. All configuration
 registers
          and the number of remaining data will be preserved. When calling again
          this function to re-enable the Stream, the transfer will be resumed
 from
          the point where it was paused.

      -@- Memory-to-Memory transfer is possible by setting the address of the
 memory into
           the Peripheral registers. In this mode, Circular mode and Double
 Buffer mode
           are not allowed.
...
#define DMA_Stream0_IT_MASK                                                    \
  (uint32_t)(DMA_LISR_FEIF0 | DMA_LISR_DMEIF0 | DMA_LISR_TEIF0 |               \
...
#define DMA_Stream4_IT_MASK                                                    \
  (uint32_t)(DMA_Stream0_IT_MASK | (uint32_t)0x20000000)
#define DMA_Stream5_IT_MASK                                                    \
  (uint32_t)(DMA_Stream1_IT_MASK | (uint32_t)0x20000000)
#define DMA_Stream6_IT_MASK                                                    \
  (uint32_t)(DMA_Stream2_IT_MASK | (uint32_t)0x20000000)
#define DMA_Stream7_IT_MASK                                                    \
  (uint32_t)(DMA_Stream3_IT_MASK | (uint32_t)0x20000000)
...
@verbatim
 ...
 ===============================================================================
    [..]
    This subsection provides functions allowing to initialize the DMA Stream
source
    and destination addresses, incrementation and data sizes, transfer
direction,
    buffer size, circular/normal mode selection, memory-to-memory mode selection
    and Stream priority value.
    [..]
    The DMA_Init() function follows the DMA configuration procedures as
described in
    reference manual (RM0090) except the first point: waiting on EN bit to be
reset.
    This condition should be checked by user application using the function
DMA_GetCmdStatus()
...
/**
  * @brief  Deinitialize the DMAy Streamx registers to their default reset
 * values.
 ...
 * in
  ...
  * @note   Before calling this function, it is recommended to check that the
 * Stream
...
DMA_Init(DMA_Stream_TypeDef* DMAy_Streamx, DMA_InitTypeDef* DMA_InitStruct)
{
  ...
  /* Clear CHSEL, MBURST, PBURST, PL, MSIZE, PSIZE, MINC, PINC, CIRC and DIR
   * bits */
 ...
 * the
  ...
  *        parameter is wrong.
  *        After disabling a DMA Stream, it is also recommended to check (or
 * wait
  *        until) the DMA Stream is effectively disabled. If a Stream is
 * disabled
...
/**
  ...
  *
  * @note   This function has no effect if the Peripheral Increment mode is
 * disabled.
  *
  * @param  DMAy_Streamx: where y can be 1 or 2 to select the DMA and x can be 0
  *          to 7 to select the DMA Stream.
  ...
  *            @arg DMA_PINCOS_WordAligned: Peripheral address increment offset
 * is
  *                                         fixed to 4 (32-bit aligned
 * addresses).
  * @retval None
  */
void
DMA_PeriphIncOffsetSizeConfig(DMA_Stream_TypeDef* DMAy_Streamx,
                              uint32_t            DMA_Pincos)
{
  /* Check the parameters */
  assert_param(IS_DMA_ALL_PERIPH(DMAy_Streamx));
...
/**
  * @brief  Configures, when the DMAy Streamx is disabled, the flow controller
 * for
  ...
  *          This parameter can be one of the following values:
  *            @arg DMA_FlowCtrl_Memory: DMAy_Streamx transactions flow
 * controller is
  *                                      the DMA controller.
 ...
 * controller
  ...
  */
void
DMA_FlowControllerConfig(DMA_Stream_TypeDef* DMAy_Streamx,
...
size
    ...
    The following function can be used to write the Stream data counter value:
      (+) void DMA_SetCurrDataCounter(DMA_Stream_TypeDef* DMAy_Streamx, uint16_t
Counter);
      -@- It is advised to use this function rather than DMA_Init() in
situations
          where only the Data buffer needs to be reloaded.
      -@- If the Source and Destination Data Sizes are different, then the value
          written in data counter, expressing the number of transfers, is
relative
    ...
    [..]
    The DMA data counter can be read to indicate the number of remaining
transfers for
...
/**
  * @brief  Writes the number of data units to be transferred on the DMAy
 * Streamx.
  * @param  DMAy_Streamx: where y can be 1 or 2 to select the DMA and x can be 0
  *          to 7 to select the DMA Stream.
...
@verbatim
 ...
 ===============================================================================
    [..]
    This subsection provides function allowing to configure and control the
double
    buffer mode parameters.

    [..]
    The Double Buffer mode can be used only when Circular mode is enabled.
    The Double Buffer mode cannot be used when transferring data from Memory to
Memory.

    [..]
    The Double Buffer mode allows to set two different Memory addresses from/to
which
    the DMA controller will access alternatively (after completing transfer
to/from
    target memory 0, it will start transfer to/from target memory 1).
    This allows to reduce software overhead for double buffering and reduce the
CPU
    access time.
...
FunctionalState NewState);
    ...
    [..]
    DMA_DoubleBufferModeConfig() is called to configure the Memory 1 base
address
    ...
    [..]
    Two functions can be called dynamically when the transfer is ongoing (or
when the DMA Stream is
    stopped) to modify on of the target Memories addresses or to check wich
Memory target is currently
    used:
    ...
    from Memory 1 then you can only modify base address of target Memory 0 and
vice versa).
    To check this condition, it is recommended to use the function
DMA_GetCurrentMemoryTarget() which
...
/**
  * @brief  Configures, when the DMAy Streamx is disabled, the double buffer
 * mode
  ...
  *
  * @note   Memory0BaseAddr is set by the DMA structure configuration in
 * DMA_Init().
  *
  * @retval None
  */
void
DMA_DoubleBufferModeConfig(DMA_Stream_TypeDef* DMAy_Streamx,
                           uint32_t Memory1BaseAddr, uint32_t DMA_CurrentMemory)
{
  /* Check the parameters */
  assert_param(IS_DMA_ALL_PERIPH(DMAy_Streamx));
...
/**
  * @brief  Enables or disables the double buffer mode for the selected DMA
 * stream.
  ...
  */
void
DMA_DoubleBufferModeCmd(DMA_Stream_TypeDef* DMAy_Streamx,
                        FunctionalState     NewState)
{
  /* Check the parameters */
  assert_param(IS_DMA_ALL_PERIPH(DMAy_Streamx));
...
/**
  ...
  *
  * @note    It is not allowed to modify the Base Address of a target Memory
 * when
  *          this target is involved in the current transfer. ie. If the DMA
 * Stream
  ...
  */
void
DMA_MemoryTargetConfig(DMA_Stream_TypeDef* DMAy_Streamx,
                       uint32_t MemoryBaseAddr, uint32_t DMA_MemoryTarget)
{
  /* Check the parameters */
  assert_param(IS_DMA_ALL_PERIPH(DMAy_Streamx));
 ...
 ===============================================================================
    ...
    [..]
      (#) DMA Enable status:
          ...
          is effectively enabled. A Stream may remain disabled if a
configuration
          parameter is wrong. After disabling a DMA Stream, it is also
recommended
    ...
    *** Polling Mode ***
    ...
    (x : DMA Stream number )
      (#) DMA_FLAG_FEIFx  : to indicate that a FIFO Mode Transfer Error event
occurred.
      (#) DMA_FLAG_DMEIFx : to indicate that a Direct Mode Transfer Error event
occurred.
      (#) DMA_FLAG_TEIFx  : to indicate that a Transfer Error event occurred.
      (#) DMA_FLAG_HTIFx  : to indicate that a Half-Transfer Complete event
occurred.
...
uint32_t DMA_FLAG);
      (+) void DMA_ClearFlag(DMA_Stream_TypeDef* DMAy_Streamx, uint32_t
DMA_FLAG);
    ...
    *** Interrupt Source ***
    ========================
    [..]
      (#) DMA_IT_FEIFx  : specifies the interrupt source for the  FIFO Mode
Transfer Error event.
      (#) DMA_IT_DMEIFx : specifies the interrupt source for the Direct Mode
Transfer Error event.
      (#) DMA_IT_TEIFx  : specifies the interrupt source for the Transfer Error
event.
      (#) DMA_IT_HTIFx  : specifies the interrupt source for the Half-Transfer
Complete event.
      (#) DMA_IT_TCIFx  : specifies the interrupt source for the a Transfer
Complete event.
    [..]
    In this Mode it is advised to use the following functions:
      (+) void DMA_ITConfig(DMA_Stream_TypeDef* DMAy_Streamx, uint32_t DMA_IT,
FunctionalState NewState);
      (+) ITStatus DMA_GetITStatus(DMA_Stream_TypeDef* DMAy_Streamx, uint32_t
DMA_IT);
      (+) void DMA_ClearITPendingBit(DMA_Stream_TypeDef* DMAy_Streamx, uint32_t
DMA_IT);
...
/**
  ...
  *
  * @note    After configuring the DMA Stream (DMA_Init() function) and enabling
  *          the stream, it is recommended to check (or wait until) the DMA
 * Stream
  *          is effectively enabled. A Stream may remain disabled if a
 * configuration
  *          parameter is wrong.
...
/**
  ...
  * @param  DMAy_Streamx: where y can be 1 or 2 to select the DMA and x can be 0
  *          to 7 to select the DMA Stream.
  * @param DMA_IT: specifies the DMA interrupt sources to be enabled or
 * disabled.
  *          This parameter can be any combination of the following values:
  ...
  */
void
DMA_ITConfig(DMA_Stream_TypeDef* DMAy_Streamx, uint32_t DMA_IT,
             FunctionalState NewState)
{
  /* Check the parameters */
  assert_param(IS_DMA_ALL_PERIPH(DMAy_Streamx));
...
/**
  * @brief  Checks whether the specified DMAy Streamx interrupt has occurred or
 * not.
  * @param  DMAy_Streamx: where y can be 1 or 2 to select the DMA and x can be 0
  *          to 7 to select the DMA Stream.
