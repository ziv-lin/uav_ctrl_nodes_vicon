...
/**
  * @brief  Initializes the DCMI according to the specified parameters in the
 * DCMI_InitStruct.
  * @param  DCMI_InitStruct: pointer to a DCMI_InitTypeDef structure that
 * contains
...
DCMI_Init(DCMI_InitTypeDef* DCMI_InitStruct)
{
  ...
  temp &= ~((uint32_t)DCMI_CR_CM | DCMI_CR_ESS | DCMI_CR_PCKPOL |
            DCMI_CR_HSPOL | DCMI_CR_VSPOL | DCMI_CR_FCRC_0 | DCMI_CR_FCRC_1 |
            DCMI_CR_EDM_0 | DCMI_CR_EDM_1);

  /* Sets the new configuration of the DCMI peripheral */
  temp |=
    ((uint32_t)DCMI_InitStruct->DCMI_CaptureMode |
     DCMI_InitStruct->DCMI_SynchroMode | DCMI_InitStruct->DCMI_PCKPolarity |
     DCMI_InitStruct->DCMI_VSPolarity | DCMI_InitStruct->DCMI_HSPolarity |
...
/**
  ...
  * @note   This function should be called before to enable and start the DCMI
 * interface.
  * @param  DCMI_CROPInitStruct:  pointer to a DCMI_CROPInitTypeDef structure
 * that
  *         contains the configuration information for the DCMI peripheral CROP
 * mode.
  * @retval None
  */
void
DCMI_CROPConfig(DCMI_CROPInitTypeDef* DCMI_CROPInitStruct)
{
  ...
  DCMI->CWSTRTR =
    (uint32_t)((uint32_t)DCMI_CROPInitStruct->DCMI_HorizontalOffsetCount |
               ((uint32_t)DCMI_CROPInitStruct->DCMI_VerticalStartLine << 16));

  /* Sets the CROP window size */
  DCMI->CWSIZER =
    (uint32_t)(DCMI_CROPInitStruct->DCMI_CaptureCount |
...
{
  DCMI->ESCR =
    (uint32_t)(DCMI_CodesInitStruct->DCMI_FrameStartCode |
...
/**
  ...
  * @note   The Crop and Embedded Synchronization features cannot be used in
 * this mode.
