...
typedef struct
{
  uint32_t USART_BaudRate; /*!< This member configures the USART communication
                              baud rate.
                                The baud rate is computed using the following
                              formula:
                              ...
                              IntegerDivider)) * 8 * (OVR8+1)) + 0.5
                                Where OVR8 is the "oversampling by 8 mode"
  ...
  uint16_t USART_Parity; /*!< Specifies the parity mode.
                              ...
                              @note When parity is enabled, the computed parity
                            is inserted
                                    at the MSB position of the transmitted data
                            (9th bit when
                                    the word length is set to 9 data bits; 8th
                            bit when the
...
#define IS_USART_ALL_PERIPH(PERIPH)                                            \
  (((PERIPH) == USART1) || ((PERIPH) == USART2) || ((PERIPH) == USART3) ||     \
   ((PERIPH) == UART4) || ((PERIPH) == UART5) || ((PERIPH) == USART6) ||       \
   ((PERIPH) == UART7) || ((PERIPH) == UART8))
...
#define USART_IT_ORE_RX                                                        \
  ((uint16_t)0x0325) /* In case interrupt is generated if the RXNEIE bit is    \
                        set */
...
#define IS_USART_GET_IT(IT)                                                    \
  (((IT) == USART_IT_PE) || ((IT) == USART_IT_TXE) || ((IT) == USART_IT_TC) || \
   ...
   ((IT) == USART_IT_ORE) || ((IT) == USART_IT_ORE_RX) ||                      \
