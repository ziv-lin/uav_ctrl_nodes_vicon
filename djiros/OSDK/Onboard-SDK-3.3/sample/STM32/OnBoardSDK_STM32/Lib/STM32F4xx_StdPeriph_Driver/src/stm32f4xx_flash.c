...
(LATENCY)
          must be correctly programmed according to the frequency of the CPU
clock
...
2.1 V   |
 |---------------|----------------|----------------|-----------------|-----------------|
 |0WS(1CPU cycle)|0 < HCLK <= 30  |0 < HCLK <= 24  |0 < HCLK <= 22   |0 < HCLK
<= 20   |
 |---------------|----------------|----------------|-----------------|-----------------|
 |1WS(2CPU cycle)|30 < HCLK <= 60 |24 < HCLK <= 48 |22 < HCLK <= 44  |20 < HCLK
<= 40  |
 |---------------|----------------|----------------|-----------------|-----------------|
 |2WS(3CPU cycle)|60 < HCLK <= 90 |48 < HCLK <= 72 |44 < HCLK <= 66  |40 < HCLK
<= 60  |
 |---------------|----------------|----------------|-----------------|-----------------|
 |3WS(4CPU cycle)|90 < HCLK <= 120|72 < HCLK <= 96 |66 < HCLK <= 88  |60 < HCLK
<= 80  |
 |---------------|----------------|----------------|-----------------|-----------------|
 |4WS(5CPU cycle)|120< HCLK <= 150|96 < HCLK <= 120|88 < HCLK <= 110 |80 < HCLK
<= 100 |
 |---------------|----------------|----------------|-----------------|-----------------|
 |5WS(6CPU cycle)|120< HCLK <= 168|120< HCLK <= 144|110 < HCLK <= 132|100 < HCLK
<= 120|
 |---------------|----------------|----------------|-----------------|-----------------|
 |6WS(7CPU cycle)|      NA        |144< HCLK <= 168|132 < HCLK <= 154|120 < HCLK
<= 140|
 |---------------|----------------|----------------|-----------------|-----------------|
 |7WS(8CPU cycle)|      NA        |      NA        |154 < HCLK <= 168|140 < HCLK
<= 160|
 +-------------------------------------------------------------------------------------+
 ...
 +-------------------------------------------------------------------------------------------------------------------+
 |               | voltage range  | voltage range  | voltage range   | voltage
range   | voltage range 2.7 V - 3.6 V |
 |               | 2.7 V - 3.6 V  | 2.4 V - 2.7 V  | 2.1 V - 2.4 V   | 1.8 V -
2.1 V   | with External Vpp = 9V      |
 |---------------|----------------|----------------|-----------------|-----------------|-----------------------------|
 |Max Parallelism|      x32       |               x16                |       x8
|          x64                |
 |---------------|----------------|----------------|-----------------|-----------------|-----------------------------|
 |PSIZE[1:0]     |      10        |               01                 |       00
|           11                |
 +-------------------------------------------------------------------------------------------------------------------+
      -@- When VOS bit (in PWR_CR register) is reset to 0 , the maximum value of
HCLK is 144 MHz.
...
this bit.
      -@- On STM32F40xx/41xx devices:
          ...
          On STM32F427x/437x devices:
           ...
           You can use PWR_MainRegulatorModeConfig() function to control VOS
bits.
...
/**
  ...
  *          For STM32F40xx/41xx and STM32F427x/437x devices this parameter can
 * be
 ...
 * parallelism.
  *          This parameter can be one of the following values:
  *            @arg VoltageRange_1: when the device voltage range is 1.8V to
 * 2.1V,
  *                                  the operation will be done by byte (8-bit)
  *            @arg VoltageRange_2: when the device voltage range is 2.1V to
 * 2.7V,
  *                                  the operation will be done by half word
 * (16-bit)
  *            @arg VoltageRange_3: when the device voltage range is 2.7V to
 * 3.6V,
  *                                  the operation will be done by word (32-bit)
  *            @arg VoltageRange_4: when the device voltage range is 2.7V to
 * 3.6V + External Vpp,
  *                                  the operation will be done by double word
 * (64-bit)
  *
  * @retval FLASH Status: The returned value can be: FLASH_BUSY,
 * FLASH_ERROR_PROGRAM,
  *                       FLASH_ERROR_WRP, FLASH_ERROR_OPERATION or
 * FLASH_COMPLETE.
  */
FLASH_Status
 ...
 * 2.7V to 3.6V.
  ...
  *
  * @param  Address: specifies the address to be programmed.
  *         This parameter can be any address in Program memory zone or in OTP
 * zone.
  * @param  Data: specifies the data to be programmed.
  * @retval FLASH Status: The returned value can be: FLASH_BUSY,
 * FLASH_ERROR_PROGRAM,
...
/**
  ...
  * @note   This function can be used within all the device supply voltage
 * ranges.
  *
  * @note   If an erase and a program operations are requested simustaneously,
  *         the erase operation is performed before the program one.
...
@verbatim
 ...
 ===============================================================================
    [..]
      ...
      (+) void FLASH_OB_UserConfig(uint8_t OB_IWDG, uint8_t OB_STOP, uint8_t
OB_STDBY)
...
/**
  ...
  *         it is not possible to program or erase the flash sector i if
 * CortexM4
  *         debug features are connected or boot code is executed in RAM, even
 * if nWRPi = 1
  * @note   Active value of nWRPi bits is inverted when PCROP mode is active
 * (SPRMOD =1).
  *
  * @param  OB_WRP: specifies the sector(s) to be write protected or
 * unprotected.
  *          This parameter can be one of the following values:
 ...
 * unprotected.
  ...
  */
void
FLASH_OB_WRP1Config(uint32_t OB_WRP, FunctionalState NewState)
{
  FLASH_Status status = FLASH_COMPLETE;
...
/**
  ...
  *
  * !!!Warning!!! When enabling OB_RDP level 2 it's no more possible to go back
 * to level 1 or 0
  *
  * @retval None
  */
...
FLASH_OB_UserConfig(uint8_t OB_IWDG, uint8_t OB_STOP, uint8_t OB_STDBY)
{
  ...
  if (status == FLASH_COMPLETE)
  {
    ...
    optiontmp =
      (uint8_t)((*(__IO uint8_t*)OPTCR_BYTE0_ADDRESS) & (uint8_t)0x0F);

    /* Update User Option Byte */
    *(__IO uint8_t*)OPTCR_BYTE0_ADDRESS =
      OB_IWDG | (uint8_t)(OB_STDBY | (uint8_t)(OB_STOP | ((uint8_t)optiontmp)));
  }
}
