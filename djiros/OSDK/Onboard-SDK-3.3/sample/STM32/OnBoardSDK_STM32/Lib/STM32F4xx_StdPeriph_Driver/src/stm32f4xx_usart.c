/**
 ...
 USART6
          RCC_APB1PeriphClockCmd(RCC_APB1Periph_USARTx, ENABLE) for USART2,
 USART3,
          UART4 or UART5.
...
#define CR1_CLEAR_MASK                                                         \
  ((uint16_t)(USART_CR1_M | USART_CR1_PCE | USART_CR1_PS | USART_CR1_TE |      \
              USART_CR1_RE))

/*!< USART CR2 register clock bits clear Mask ((~(uint16_t)0xF0FF)) */
#define CR2_CLOCK_CLEAR_MASK                                                   \
  ((uint16_t)(USART_CR2_CLKEN | USART_CR2_CPOL | USART_CR2_CPHA |              \
              USART_CR2_LBCL))
...
written
             in the data register is transmitted but is changed by the parity
bit.
...
(RM0090)).
     ...
     (+) For the synchronous mode in addition to the asynchronous mode
parameters these
 ...
 * values.
  * @param  USARTx: where x can be 1, 2, 3, 4, 5, 6, 7 or 8 to select the USART
 * or
  *         UART peripheral.
  * @retval None
  */
...
USART_Init(USART_TypeDef* USARTx, USART_InitTypeDef* USART_InitStruct)
{
  ...
  /* The hardware flow control is available only for USART1, USART2, USART3 and
   * USART6 */
...
/**
  ...
  * @param  USART_InitStruct: pointer to a USART_InitTypeDef structure which
 * will
  *         be initialized.
  * @retval None
  */
...
USART_OneBitMethodCmd(USART_TypeDef* USARTx, FunctionalState NewState)
{
  ...
  {
    /* Disable the one bit method by clearing the ONEBITE bit in the CR3
     * register */
...
register
    stores the data in the TDR register and which is copied in the shift
register
    at the end of the current transmission.
    [..]
    The read access of the USART_DR register can be done using the
USART_ReceiveData()
    function and returns the RDR buffered value. Whereas a write access to the
USART_DR
...
@verbatim
 ...
 ===============================================================================
    ...
    master.
    [..]
    USART multiprocessor communication is possible through the following
procedure:
 ...
 * or
  *         UART peripheral.
  * @param  USART_LINBreakDetectLength: specifies the LIN break detection
 * length.
  *          This parameter can be one of the following values:
...
where
    ...
    USART Half duplex communication is possible through the following procedure:
      (#) Program the Baud rate, Word length, Stop bits, Parity, Mode
transmitter
          or Mode receiver and hardware flow control values using the
USART_Init()
          function.
      (#) Configures the USART address using the USART_SetAddress() function.
      (#) Enable the USART using the USART_Cmd() function.
...
Smartcards as
    ...
    In smartcard mode, SCLK is not associated to the communication but is simply
derived
...
function.
      (#) Program the USART clock using the USART_ClockInit() function as
following:
        (++) USART Clock enabled
        (++) USART CPOL Low
...
following:
        ...
        (++) Tx and Rx enabled
      (#) POptionally you can enable the parity error interrupt using the
USART_ITConfig()
          function
      (#) PEnable the USART using the USART_Cmd() function.
      (#) PEnable the Smartcard NACK using the USART_SmartCardNACKCmd()
function.
      (#) PEnable the Smartcard interface using the USART_SmartCardCmd()
function.
...
recommended
          to use 1.5 stop bits for both transmitting and receiving to avoid
switching
          between the two configurations.
      -@- In smartcard mode, the following bits must be kept cleared:
        (+@) LINEN bit in the USART_CR2 register.
        (+@) HDSEL and IREN bits in the USART_CR3 register.
      -@- Smartcard mode is available on USART peripherals only (not available
on UART4
...
any data
    on the IrDA receive line will be ignored by the IrDA decoder and if the
Receiver
    is busy, data on the TX from the USART to IrDA will not be encoded by IrDA.
    While receiving data, transmission should be avoided as the data to be
transmitted
    could be corrupted.
    [..]
...
Transmitter/Receiver
          modes and hardware flow control values using the USART_Init()
function.
      (#) Enable the USART using the USART_Cmd() function.
...
mode
      ...
      -@- A pulse of width less than two and greater than one PSC period(s) may
or may
          not be rejected.
      -@- The receiver set up time should be managed by software. The IrDA
physical layer
          specification specifies a minimum of 10 ms delay between transmission
and
...
manage
    ...
    *** Polling Mode ***
    ...
    In Polling Mode, the SPI communication can be managed by 10 flags:
      (#) USART_FLAG_TXE : to indicate the status of the transmit buffer
register
      (#) USART_FLAG_RXNE : to indicate the status of the receive buffer
register
...
interrupt.
        (##) USART_IT_CTS : specifies the interrupt source for the CTS
interrupt.
        (##) USART_IT_LBD : specifies the interrupt source for the LIN break
detection
                            interrupt.
        (##) USART_IT_PE : specifies the interrupt source for the parity error
interrupt.
 ...
 * or
  ...
  *          This parameter can be one of the following values:
  *            @arg USART_FLAG_CTS:  CTS Change flag (not available for UART4
 * and UART5)
 ...
 * and UART5).
  ...
  *            @arg USART_FLAG_RXNE: Receive data register not empty flag.
  *
  * @note   PE (Parity error), FE (Framing error), NE (Noise error), ORE
 * (OverRun
 ...
 * to
  *          USART_SR register (USART_GetFlagStatus()) followed by a write
 * operation
 ...
 * or
  ...
  *          This parameter can be one of the following values:
  *            @arg USART_IT_CTS:  CTS change interrupt (not available for UART4
 * and UART5)
  *            @arg USART_IT_LBD:  LIN Break detection interrupt
  *            @arg USART_IT_TXE:  Transmit Data Register empty interrupt
  *            @arg USART_IT_TC:   Transmission complete interrupt
  *            @arg USART_IT_RXNE: Receive Data register not empty interrupt
  *            @arg USART_IT_IDLE: Idle line detection interrupt
  *            @arg USART_IT_ORE_RX : OverRun Error interrupt if the RXNEIE bit
 * is set
  *            @arg USART_IT_ORE_ER : OverRun Error interrupt if the EIE bit is
 * set
 ...
 * register
  *          (USART_ReceiveData()).
  * @note   RXNE pending bit can be also cleared by a read to the USART_DR
 * register
  *          (USART_ReceiveData()).
  * @note   TC pending bit can be also cleared by software sequence: a read
  *          operation to USART_SR register (USART_GetITStatus()) followed by a
 * write
