...
{
  uint32_t
    DMA_Channel; /*!< Specifies the channel used for the specified stream.
                      This parameter can be a value of @ref DMA_channel */

  uint32_t DMA_PeripheralBaseAddr; /*!< Specifies the peripheral base address
                                      for DMAy Streamx. */

  uint32_t DMA_Memory0BaseAddr; /*!< Specifies the memory 0 base address for
                                   DMAy Streamx.
                                     This memory is the default memory used when
                                   double buffer mode is
                                     not enabled. */

  uint32_t DMA_DIR; /*!< Specifies if the data will be transferred from memory
                       to peripheral,
  ...
  uint32_t DMA_BufferSize; /*!< Specifies the buffer size, in data unit, of the
                              specified Stream.
                                The data unit is equal to the configuration set
                              in DMA_PeripheralDataSize
  ...
  uint32_t DMA_Mode; /*!< Specifies the operation mode of the DMAy Streamx.
                        ...
                        memory-to-memory
                                data transfer is configured on the selected
                        Stream */
  ...
  uint32_t DMA_FIFOMode; /*!< Specifies if the FIFO mode or Direct mode will be
                            used for the specified Stream.
                               This parameter can be a value of @ref
                            DMA_fifo_direct_mode
                               @note The Direct mode (FIFO mode disabled) cannot
                            be used if the
                                      memory-to-memory data transfer is
  ...
  uint32_t DMA_MemoryBurst; /*!< Specifies the Burst transfer configuration for
                               the memory transfers.
                                 It specifies the amount of data to be
                               transferred in a single non interruptable
                                 transaction. This parameter can be a value of
                               @ref DMA_memory_burst
                                 @note The burst mode is possible only if the
                               address Increment mode is enabled. */

  uint32_t DMA_PeripheralBurst; /*!< Specifies the Burst transfer configuration
                                   for the peripheral transfers.
                                     It specifies the amount of data to be
                                   transferred in a single non interruptable
                                     transaction. This parameter can be a value
                                   of @ref DMA_peripheral_burst
                                     @note The burst mode is possible only if
...
#define IS_DMA_PRIORITY(PRIORITY)                                              \
  (((PRIORITY) == DMA_Priority_Low) || ((PRIORITY) == DMA_Priority_Medium) ||  \
   ((PRIORITY) == DMA_Priority_High) || ((PRIORITY) == DMA_Priority_VeryHigh))
/**
  * @}
  */
...
#define IS_DMA_MEMORY_BURST(BURST)                                             \
  (((BURST) == DMA_MemoryBurst_Single) || ((BURST) == DMA_MemoryBurst_INC4) || \
...
#define IS_DMA_FIFO_STATUS(STATUS)                                             \
  (((STATUS) == DMA_FIFOStatus_Less1QuarterFull) ||                            \
   ...
   ((STATUS) == DMA_FIFOStatus_Full) || ((STATUS) == DMA_FIFOStatus_Empty))
/**
  * @}
  */
