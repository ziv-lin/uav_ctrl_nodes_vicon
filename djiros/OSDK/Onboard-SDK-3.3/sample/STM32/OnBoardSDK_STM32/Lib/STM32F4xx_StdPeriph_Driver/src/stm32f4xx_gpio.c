 ...
 [..]
   ...
   (#) Configure the GPIO pin(s) using GPIO_Init()
       ...
       (++) Output: Push-Pull (Pull-up, Pull-down or no Pull)
            Open Drain (Pull-up, Pull-down or no Pull). In output mode, the
speed
...
/**
  ...
  *            @arg GPIO_AF_RTC_50Hz: Connect RTC_50Hz pin to AF0 (default after
 * reset)
  *            @arg GPIO_AF_MCO: Connect MCO pin (MCO1 and MCO2) to AF0 (default
 * after reset)
  *            @arg GPIO_AF_TAMPER: Connect TAMPER pins (TAMPER_1 and TAMPER_2)
 * to AF0 (default after reset)
  *            @arg GPIO_AF_SWJ: Connect SWJ pins (SWD and JTAG)to AF0 (default
 * after reset)
  *            @arg GPIO_AF_TRACE: Connect TRACE pins to AF0 (default after
 * reset)
  ...
  *            @arg GPIO_AF_OTG_HS_FS: Connect OTG HS (configured in FS) pins to
 * AF12
...
GPIO_PinAFConfig(GPIO_TypeDef* GPIOx, uint16_t GPIO_PinSource, uint8_t GPIO_AF)
{
  ...
  GPIOx->AFR[GPIO_PinSource >> 0x03] &=
    ~((uint32_t)0xF << ((uint32_t)((uint32_t)GPIO_PinSource & (uint32_t)0x07) *
                        4));
