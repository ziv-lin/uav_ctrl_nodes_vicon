/**
 ...
 RCC_APB1PeriphClockCmd(RCC_APB1Periph_I2Cx, ENABLE)
      ...
      (#) Program the Mode, duty cycle , Own address, Ack, Speed and
 Acknowledged
          Address using the I2C_Init() function.

      (#) Optionally you can enable/configure the following parameters without
          re-initialization (i.e there is no need to call again I2C_Init()
 function):
        (++) Enable the acknowledge feature using I2C_AcknowledgeConfig()
 function
        (++) Enable the dual addressing mode using I2C_DualAddressCmd() function
        (++) Enable the general call using the I2C_GeneralCallCmd() function
        (++) Enable the clock stretching using I2C_StretchClockCmd() function
        (++) Enable the fast mode duty cycle using the
 ...
 function
          (+++) Configure the SMBusAlert pin using I2C_SMBusAlertConfig()
 function
      ...
      (#) When using the DMA mode
        ...
        -@@- When using DMA mode, I2C interrupts may be used at the same time to
             control the communication flow (Start/Stop/Ack... events and
 errors).
...
/**
  * @brief  Deinitialize the I2Cx peripheral registers to their default reset
 * values.
...
I2C_Init(I2C_TypeDef* I2Cx, I2C_InitTypeDef* I2C_InitStruct)
{
  ...
  /* To use the I2C at 400 KHz (in fast mode), the PCLK1 frequency (I2C
     peripheral
  ...
  else /*(I2C_InitStruct->I2C_ClockSpeed <= 400000)*/
  {
    ...
    I2Cx->TRISE =
      (uint16_t)(((freqrange * (uint16_t)300) / (uint16_t)1000) + (uint16_t)1);
  }
  ...
  I2Cx->OAR1 =
    (I2C_InitStruct->I2C_AcknowledgedAddress | I2C_InitStruct->I2C_OwnAddress1);
}

/**
  * @brief  Fills each I2C_InitStruct member with its default value.
  * @param  I2C_InitStruct: pointer to an I2C_InitTypeDef structure which will
 * be initialized.
  * @retval None
  */
void
...
/**
  ...
  */
void
I2C_AnalogFilterCmd(I2C_TypeDef* I2Cx, FunctionalState NewState)
{
  /* Check the parameters */
  assert_param(IS_I2C_ALL_PERIPH(I2Cx));
  assert_param(IS_FUNCTIONAL_STATE(NewState));
...
/**
  ...
  */
void
I2C_DigitalFilterConfig(I2C_TypeDef* I2Cx, uint16_t I2C_DigitalFilter)
{
  uint16_t tmpreg = 0;
...
/**
  ...
  *          This parameter can be one of the following values:
  *            @arg I2C_NACKPosition_Next: indicates that the next byte will be
 * the last
  *                                        received byte.
  *            @arg I2C_NACKPosition_Current: indicates that current byte is the
 * last
  *                                           received byte.
  *
  * @note    This function configures the same bit (POS) as
 * I2C_PECPositionConfig()
  *          but is intended to be used in I2C mode while
 * I2C_PECPositionConfig()
 ...
 * I2C_NACKPositionConfig()
  *          but is intended to be used in SMBUS mode while
 * I2C_NACKPositionConfig()
 ...
 ===============================================================================
    ...
    depending on the application requirements and constraints:
     ...
     (#) Basic state monitoring (Using I2C_CheckEvent() function)
        ...
        and returns ERROR if one or more flags are missing in the current
status.

          (++) When to use
             (+++) This function is suitable for most applications as well as
for startup
               activity since the events are fully described in the product
reference
               manual (RM0090).
             (+++) It is also suitable for users who need to define their own
events.

          (++) Limitations
               If an error occurs (ie. error flags are set besides to the
monitored
...
shifted left
        by 16 bits and concatenated to Status Register 1).
          ...
          (++) When to use
             (+++) This function is suitable for the same applications above but
it
               allows to overcome the mentioned limitation of
I2C_GetFlagStatus()
               function.
             (+++) The returned value could be compared to events already
defined in
               the library (stm32f4xx_i2c.h) or to custom values defined by
user.
               This function is suitable when multiple flags are monitored at
the
               same time.
             (+++) At the opposite of I2C_CheckEvent() function, this function
allows
               user to choose when an event is accepted (when all events flags
are
               set and no other flags are set or just when the needed flags are
set
...
this
               function if user decides to check only regular communication
flags
     ...
     (#) Flag-based state monitoring (Using the function I2C_GetFlagStatus())
      ...
      one single flag (ie. I2C_FLAG_RXNE ...).
          ...
          (++) When to use
             (+++) This function could be used for specific applications or in
debug
               phase.
             (+++) It is suitable when only one flag checking is needed (most
I2C
          ...
          (++) Limitations:
             ...
             (+++) Function may need to be called twice or more in order to
monitor
               one single event.
...
/**
  ...
  *            @arg I2C_FLAG_BERR: Bus error flag
  *
  * @note   STOPF (STOP detection) is cleared by software sequence: a read
 * operation
 ...
 * operation
  ...
  * @note   ADDR (Address sent) is cleared by software sequence: a read
 * operation to
  *          I2C_SR1 register (I2C_GetFlagStatus()) followed by a read operation
 * to
  *          I2C_SR2 register ((void)(I2Cx->SR2)).
  * @note   SB (Start Bit) is cleared software sequence: a read operation to
 * I2C_SR1
  *          register (I2C_GetFlagStatus()) followed by a write operation to
 * I2C_DR
...
/**
  ...
  * @note   STOPF (STOP detection) is cleared by software sequence: a read
 * operation
  *          to I2C_SR1 register (I2C_GetITStatus()) followed by a write
 * operation to
  *          I2C_CR1 register (I2C_Cmd() to re-enable the I2C peripheral).
  * @note   ADD10 (10-bit header sent) is cleared by software sequence: a read
  *          operation to I2C_SR1 (I2C_GetITStatus()) followed by writing the
 * second
  ...
  * @note   ADDR (Address sent) is cleared by software sequence: a read
 * operation to
  *          I2C_SR1 register (I2C_GetITStatus()) followed by a read operation
 * to
  *          I2C_SR2 register ((void)(I2Cx->SR2)).
  * @note   SB (Start Bit) is cleared by software sequence: a read operation to
  *          I2C_SR1 register (I2C_GetITStatus()) followed by a write operation
 * to
