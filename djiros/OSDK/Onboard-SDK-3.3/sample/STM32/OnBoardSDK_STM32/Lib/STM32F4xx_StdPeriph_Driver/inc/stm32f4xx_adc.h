...
{
  uint32_t
    ADC_Resolution; /*!< Configures the ADC resolution dual mode.
                         This parameter can be a value of @ref ADC_resolution */
  FunctionalState
    ADC_ScanConvMode;                     /*!< Specifies whether the conversion
  ...
  FunctionalState ADC_ContinuousConvMode; /*!< Specifies whether the conversion
                                             ...
                                             ENABLE or DISABLE. */
  uint32_t
    ADC_ExternalTrigConvEdge; /*!< Select the external trigger edge and
                                 ...
                                 */
  uint32_t
    ADC_ExternalTrigConv;      /*!< Select the external event used to trigger
...
{
  uint32_t
    ADC_Mode; /*!< Configures the ADC to operate in
                   independent or multi mode.
                   This parameter can be a value of @ref ADC_Common_mode */
  uint32_t
    ADC_Prescaler; /*!< Select the frequency of the clock
                        to the ADC. The clock is common for all the ADCs.
                        This parameter can be a value of @ref ADC_Prescaler */
  uint32_t
    ADC_DMAAccessMode; /*!< Configures the Direct memory access
...
#define IS_ADC_MODE(MODE)                                                      \
  (((MODE) == ADC_Mode_Independent) ||                                         \
   ...
   ((MODE) == ADC_DualMode_RegSimult) || ((MODE) == ADC_DualMode_Interl) ||    \
...
#define IS_ADC_PRESCALER(PRESCALER)                                            \
  (((PRESCALER) == ADC_Prescaler_Div2) ||                                      \
   ...
   ((PRESCALER) == ADC_Prescaler_Div6) || ((PRESCALER) == ADC_Prescaler_Div8))
/**
  * @}
  */
...
#define ADC_DMAAccessMode_Disabled                                             \
  ((uint32_t)0x00000000) /* DMA mode disabled */
#define ADC_DMAAccessMode_1                                                    \
  ((uint32_t)0x00004000) /* DMA mode 1 enabled (2 / 3 half-words one by one -  \
                            1 then 2 then 3)*/
#define ADC_DMAAccessMode_2                                                    \
  ((uint32_t)0x00008000) /* DMA mode 2 enabled (2 / 3 half-words by pairs -    \
                            2&1 then 1&3 then 3&2)*/
#define ADC_DMAAccessMode_3                                                    \
  ((uint32_t)0x0000C000) /* DMA mode 3 enabled (2 / 3 bytes by pairs - 2&1     \
                            then 1&3 then 3&2) */
#define IS_ADC_DMA_ACCESS_MODE(MODE)                                           \
  (((MODE) == ADC_DMAAccessMode_Disabled) ||                                   \
   ((MODE) == ADC_DMAAccessMode_1) || ((MODE) == ADC_DMAAccessMode_2) ||       \
